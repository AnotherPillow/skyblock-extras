// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @description A userscript to improve the skyblock.net forums experience!
// @match       https://skyblock.net/*
// @version     1.3.2
// @author      AnotherPillow
// @license     GNU GPLv3
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// @downloadURL https://anotherpillow.github.io/skyblock-extras/build/sbe.min.js
// ==/UserScript==
"use strict";window.o=XenForo||window.o||{},globalThis.o=XenForo||globalThis.o||window.o||{};let SELECTED_VANILLA_THEME=localStorage.getItem("sbe-vanilla-style");function waitForElm(r){return new Promise(t=>{if(document.querySelector(r))return t(document.querySelector(r));const e=new MutationObserver(o=>{document.querySelector(r)&&(e.disconnect(),t(document.querySelector(r)))});e.observe(document.body,{childList:!0,subtree:!0})})}function between(o,t,e){return t<=o&&o<=e}
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(o,t){for(var e=t;1<e;e--)o%e==0&&t%e==0&&(o/=e,t/=e);return o+":"+t}const AF=Array.from,colonNumber=(o,e)=>o.filter((o,t)=>t<e).filter(o=>""!==o),getMonthFromString=o=>new Date(Date.parse(o+" 1, 2012")).getMonth()+1,getHrefWithoutAnchor=()=>window.location.href.replace(new RegExp(window.location.hash+"$"),""),isOnThread=getHrefWithoutAnchor().match(/https\:\/\/skyblock\.net\/threads\/.+\.\d+\/?/),isInConversation=getHrefWithoutAnchor().match(/https\:\/\/skyblock\.net\/conversations\/.+\.\d+\/?/),isOnUserProfile=(window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1)&&!window.location.search.startsWith("?card"),presentUserProfile=isOnUserProfile?window.location.pathname.split("/").at(-1).split("."):null,isOnUserCard=(window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1)&&window.location.search.startsWith("?card"),isOnIndex="https://skyblock.net/"==window.location.href,isOnOriginalTheme=!document.querySelector('.social-row>[href="https://www.reddit.com/r/SkyBlock"]')&&document.querySelector('.pageContent>span>a[href="http://blackcaffeine.com/"]'),isOnMiddleTheme=!document.querySelector('a[href="http://blackcaffeine.com/"]')&&!document.querySelector('.social-row>[href="https://www.reddit.com/r/SkyBlock"]'),isOnNewTheme=document.querySelector('.link-row>a[href="/how-to-install-skyblock"]')&&document.querySelector("#footer>.top>.container>.col>p"),$import=o=>(alert("If you are seeing this, something has gone very wrong."),""),xfToken=XenForo._csrfToken,ls=localStorage,addStyle=window.GM_addStyle||GM.addStyle||(o=>{var t=document.createElement("style");t.id="sbe-style-"+Math.random(),t.innerHTML=o,document.head.appendChild(t)});function patchClass(o,t,e){if(void 0===o)return console.log(`Cannot patch ${t} of ${o}.`);const r=o[t];console.log("Patching "+t,r),o[t]=function(...o){return e.call(this,r.bind(this),...o)}}
// adapted from https://stackoverflow.com/a/9160869
addStyle(/*$import*/'.sbe-pointer {    cursor: pointer;}#sbe-settings-modal {    width: 640px;    height: 560px;    border-radius: 1em;    border: medium;    outline: none;    text-align: center;    scrollbar-width: none;}.navTabs {    position: relative;}.navTabs::before {    content: "+";    position: absolute;    z-index: 20;    color: white;    top: -0.7rem;    left: 5px;    font-size: 3em;}.sbe-strikethrough {    text-decoration: line-through !important;    text-decoration-thickness: 2px !important;}.message .messageMeta {    width: 95%;}');const insert=(o,t,e)=>0<t?o.substring(0,t)+e+o.substring(t,o.length):e+o;class _Settings{threadTitleEnabled=!0;hideShopTab=!0;strikethroughBannedUsers=!0;SBonlIntegration=!0;actualDateOnFrontpage=!0;fixBedrockPlayersImages=!0;responsiveModals=!0;movePoke=!0;ratingRatio=!0;removeRatingCommas=!0;avatarOnProfileStats=!1;birthdayHatOnPFP=!0;roundedFriendsOnProfile=!0;postLinkButton=!0;minotarNotCrafatar=!0;noMoreCamo=!1;fadeInReactions=!0;adBlocker=!0;moreSearchOnCard=!0;unpinLawsuit=!0;fixOldLinks=!0;dontShare=!0;copyMessageBBCodeButton=!0;imgurProxyEnabled=!1;rimgoInstanceHost="https://ri.nadeko.net";t;i(o,t){var e="sbe-setting-"+t.toString().replace(/\s/g,"_"),r=document.createElement("label"),o=(r.innerHTML=o+": ",r.setAttribute("for",e),document.createElement("input"));o.name=e,o.id=e,"boolean"==typeof this[t]?(o.type="checkbox",o.checked=this[t]??!1,o.addEventListener("click",o=>{o=//@ts-ignore
o.target.checked;this[t]=o})):(o.type="text",o.defaultValue=this[t]??"",o.addEventListener("change",o=>{o=//@ts-ignore
o.target.value;this[t]=o})),this.t?.appendChild(r),this.t?.appendChild(o),this.l()}l(){this.t?.appendChild(document.createElement("br"))}constructor(){this.m(),this.t=document.createElement("dialog"),this.t.id="sbe-settings-modal";var o=document.createElement("h1"),o=(o.innerHTML="Skyblock Extras Settings",o.style.fontSize="2em",this.t.appendChild(o),this.l(),this.i("Thread Title as Browser Title","threadTitleEnabled"),this.i("Remove the shop tab","hideShopTab"),this.i("Strike through banned users' names","strikethroughBannedUsers"),this.i("Skyblock.onl Integration","SBonlIntegration"),this.i("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.i("Fix bedrock players' images","fixBedrockPlayersImages"),this.i("Responsive Modals","responsiveModals"),this.i("Move poking out of moderator tools","movePoke"),this.i("Add given:received ratio for reactions on profile","ratingRatio"),this.i("Remove commas from ratings","removeRatingCommas"),this.i("Avatar on profile stats","avatarOnProfileStats"),this.i("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),this.i("Round friends' icons on profile","roundedFriendsOnProfile"),this.i("Add button to copy link to post on posts","postLinkButton"),this.i("Replace Craftar with Minotar","minotarNotCrafatar"),this.i("Remove Skyblock's image proxy","noMoreCamo"),this.i("Fade in reaction opacity on hover","fadeInReactions"),this.i("Block Ads","adBlocker"),this.i("More search options on member card","moreSearchOnCard"),this.i("Unpin Lawsuit","unpinLawsuit"),this.i("Fix old forum links","fixOldLinks"),this.i("Remove share buttons","dontShare"),this.i("Add a button to copy message BBCode","copyMessageBBCodeButton"),this.i("Circumvent imgur blocks","imgurProxyEnabled"),this.i("Rimgo instance host (for imgur access)","rimgoInstanceHost"),document.createElement("button")),o=(o.innerHTML="Save",o.style.width="6em",o.style.height="2.5em",o.addEventListener("click",o=>{this.A(),XenForo.alert("Settings have been saved.",!1,5e3,console.log),setTimeout(()=>{window.location.reload()},4500)}),this.l(),this.l(),this.t.appendChild(o),document.createElement("button")),o=(o.innerHTML="Close",o.style.width="6em",o.style.height="2.5em",o.addEventListener("click",o=>{this.close()}),this.l(),this.l(),this.l(),this.l(),this.t.appendChild(o),document.body.appendChild(this.t),document.createElement("img"));o.src="data:image/png;base64,",o.id="sbe-settings-opener-img",
/* opener.style.marginLeft = '9em' */
o.style.marginTop="2px",o.style.marginLeft="2em",o.height=20,o.width=20,o.addEventListener("click",o=>{this.open()}),isOnMiddleTheme||isOnOriginalTheme?document.querySelector('[class="navTabs"]')?.insertBefore(o,document.querySelector(".visitorTabs")):document.querySelector("#topbar>.p-nav-inner>.left")?.appendChild(o)}open(){this.t?.showModal()}close(){this.t?.close()}A(){localStorage.setItem("sbe-settings",JSON.stringify({threadTitleEnabled:this.threadTitleEnabled,hideShopTab:this.hideShopTab,strikethroughBannedUsers:this.strikethroughBannedUsers,SBonlIntegration:this.SBonlIntegration,actualDateOnFrontpage:this.actualDateOnFrontpage,fixBedrockPlayersImages:this.fixBedrockPlayersImages,responsiveModals:this.responsiveModals,movePoke:this.movePoke,ratingRatio:this.ratingRatio,removeRatingCommas:this.removeRatingCommas,avatarOnProfileStats:this.avatarOnProfileStats,birthdayHatOnPFP:this.birthdayHatOnPFP,roundedFriendsOnProfile:this.roundedFriendsOnProfile,postLinkButton:this.postLinkButton,minotarNotCrafatar:this.minotarNotCrafatar,noMoreCamo:this.noMoreCamo,fadeInReactions:this.fadeInReactions,moreSearchOnCard:this.moreSearchOnCard,adBlocker:this.adBlocker,unpinLawsuit:this.unpinLawsuit,fixOldLinks:this.fixOldLinks,dontShare:this.dontShare,copyMessageBBCodeButton:this.copyMessageBBCodeButton,imgurProxyEnabled:this.imgurProxyEnabled,rimgoInstanceHost:this.rimgoInstanceHost}))}m(){var o=JSON.parse(localStorage.getItem("sbe-settings")??"{}");for(const t of Object.keys(o))this[t]=o[t]}}const settings=new _Settings,THEMES={g:1,p:6,u:22,NEW:30,k:31,h:16};let themes=[{name:"Dark Mode (Pink Accent)",description:"A dark mode & pink accented theme",css:/*$import*/'#content .pageContent {    background-color: #414141;}    .secondaryContent,    .avatar img,    .avatarCropper,    .recentNews,    .breadBoxTop>nav>fieldset.breadcrumb,    .breadBoxBottom>nav>fieldset.breadcrumb,    .breadcrumb .crust a.crumb,    #searchBar,    .messageUserBlock,    div.section.sectionMain,    div.messageUserBlock>.avatarHolder,    ul.tabs.mainTabs.Tabs li{    background: #2D2D2DBB !important;    color: #c5c5c5;    /* outline: 1px solid lime; */}    .messageContent,    .section.sectionMain.recentNews,    .recentNews>div.primaryContent.leftDate,    fieldset#QuickSearch,    fieldset#QuickSearch>formPopup,    .footer .pageContent,    .navTabs,    div.primaryControls,    input#QuickSearchQuery,    ul.tabs.mainTabs.Tabs,    ul.tabs.mainTabs.Tabs>li,    ul.tabs.mainTabs.Tabs>li>a,    form#ProfilePoster,    li[id^="profile-post-"],    .primaryContent,    #AccountMenu,    .mainContent>ul.tabs{    background: #323232;}.messageInfo>textarea.textCtrl:focus {    background-image: none !important;    background: #665766;}    .messageInfo>textarea.textCtrl ,    #AccountMenu>.menuHeader,    ol#forums,    ol.nodeList,    .mainContent>ul.tabs>li:not(.active)>a{    background-color: #2b2b2b;}    li.node>ol.nodeList>li.node>div.nodeInfo,    div.extraUserInfo{    background-color: #1a1a1a !important;}    .titleBar,    #serverstatus,    .visitorText,    .dark_postrating_neutral,    .sectionFooter>div.continue>a.iconKey.button,    footer>.footerLegal,    footer>.footerLegal>a,    input#QuickSearchQuery,    article>blockquote.ugc.baseHtml,    .tabs>li:not(.active)>a,    div.primaryContent.menuHeader>h3>a.concealed[href^="members/"],    ol.nodeList>li.node>div.nodeInfo,    div#pageNodeContent,    .searchResult.post>.listBlock.main>blockquote>a{    color: white !important;}    a,    .PageNavNext,    [class="PageNavNext "],    a.PreviewTooltip,    .primaryContent a,    ul.Tabs.tabs.mainTabs>li.active>a,    #AccountMenu a,    #AccountMenu .secondaryContent a,    #AccountMenu .AutoValidator label{    color: #c450c6;}    .navTabs .navTab.selected .tabLinks,    .subHeading,    .sectionFooter,    .PageNav>nav a[href^="articles/"],    li.navTab.selected,    li.node.category > div.nodeInfo,    .discord-widget > p.discord-join > a,    .mainContent>ul.tabs>li.active>a,    ul[id^="premium-"].staffTitle{    background-color: #d38fb9;}    #header,     #headerMover #headerProxy,     #content,    .sectionFooter>div.continue>a.iconKey.button,    footer    /* nav span.scrollable>span.items a[href] */{    background: #7d4f77;}    .subHeading,    li.node.category > div.nodeInfo{    border-bottom: none !important;    border-top: none !important;}.PageNav>nav a[href^="articles/"] {    color: black;}a.avatar>img[alt][src^="data/avatars/"] {    filter: brightness(0.8);}    span.arrow,    span.arrow>span{    border-left-color: thistle !important;}.discussionListItems .unread .title a {    background: url(https://static.skyblock.net/sparkles/sparkle-7.gif);}',basis:THEMES.p},{name:"Nightblock",description:"A dark theme for Skyblock Forums",css:/*$import*/'/*This file has been dedicated to the public domain by the original author nightcat (https://nightcat.gg/). Explicit permission for use in skyblock-extras has been received, regardless.All changes from the original (https://github.com/uso-archive/data/blob/flomaster/data/usercss/146532.user.css), are licensed GPL 3.0.*//* ==UserStyle==@name         Nightblock@namespace    USO Archive@author       Nightcat@description  A dark theme for Skyblock Forums.@version      20200211.16.37@license      CC0-1.0@preprocessor uso==/UserStyle== */body,    .button,    .staffTitle li,    .username .style2,    .username .style24,    .dark_postrating_neutral,    span[style="color: #000000"],    span[style="color: rgb(0, 0, 0)"],    .bbCodeQuote .attribution,    .event .content .description em,    .discussionList .sectionFooter .contentSummary {    color: #fff !important;}a {    color: #bb86fc !important;}.formPopup .controlsWrapper,    .subHeading,    .textWithCount.subHeading .text,    .discussionList .sticky .title a,    .DateTime,    .discussionList .sticky .EditControl,    .discussionList .sticky .username,    .discussionList .sticky .pairsJustified dt,    a.username.author,    .postNumber,    .discussionList .sticky .DateTime,    .UnhiddenContent > .attribution.type{    color: #03dac5 !important;}#header,    body,    .pageContent,    .mainContainer,    .navTabs,    .secondaryContent,    .breadcrumb,    .sectionMain,    .primaryContent,    .button,    .profilePage .tabs.mainTabs,    .profilePage .tabs.mainTabs li a,    .nodeInfo.categoryForumNodeInfo,    .discussionListItem .posterAvatar .avatarContainer,    .discussionListItem,    .discussionListItem .posterAvatar,    .discussionListItem .stats,    .dark_postrating_detail,    .bbCodeQuote .quoteContainer,    .bbCodeBlock pre,    .bbCodeBlock .code,    .tabs,    .tabs li a {    background: #121212 !important;}.stackAlertContent,    .navTabs .navTab.selected .tabLinks,    .navTabs .navTab.selected .navLink,    .navPopup .PopupItemLinkActive:hover,    #QuickSearch,    .primaryControls,    .secondaryControls,    .formPopup .controlsWrapper,    a.crumb,    .subHeading,    .sectionFooter,    .avatar img,    .PageNav a,    .nodeList .categoryStrip,    .staffTitle,    .profilePage .tabs.mainTabs li.active a,    .sectionHeaders,    .discussionList .sticky .posterAvatar,    .discussionList .sticky .stats,    .discussionList .sticky,    .messageUserBlock div.avatarHolder,    .messageUserBlock,    .messageUserBlock .extraUserInfo,    .bbCodeQuote .attribution,    .bbCodeBlock .type,    .redactor_toolbar,    .tabs li.active a {    background: #1f1b24 !important;}.breadcrumb .crust .arrow span {    border-left-color: #1f1b24 !important;}.subHeading,    .sectionFooter,    .nodeList .categoryStrip,    .sectionHeaders {    border: 1px solid #d7edfc !important;}.navTabs .navTab.selected .navLink {    text-shadow: 0 0 0 transparent, 0px 0px 3px #000;}.Menu {    border: 1px solid #1f1b24 !important;    border-top: 5px solid #1f1b24 !important;}.discussionList .sticky .title a,    .discussionList .sticky .DateTime,    .discussionList .sticky .EditControl,    .discussionList .sticky .username,    .discussionList .sticky .pairsJustified dt {    text-shadow: none !important;}#friends-mount>.friend {    background-color: inherit;}.UnhiddenContent blockquote,    .deletedNotice {    color: black;}',basis:THEMES.p},{name:"Better New SB",description:"A better version of the new Skyblock theme",css:/*$import*/'div.navTabs {    background:#2b485c;    border-radius: 0 !important;}#landingHero>* {    display:none !important;    background: none !important;}#landingHero {    height:25px;    padding:0 !important;}#content .sidebar .section {    border-radius: 0px !important;}#content .section {    -webkit-box-shadow:none !important;    box-shadow:none !important;}.avatar img, .avatarWrap .img.s {    border-radius: 5px !important;}.visitorTabs, .navTabs .visitorTabs {    display: block !important;    }div#navigation {    border-bottom: none;}#content .sidebar .section .secondaryContent {    padding: 15px !important;}#footer>.top {    padding:25px;}#content {    background: #d1eef5 !important;}.newsText {    color: #113240}li[id^="thread"]>.title {    font-size: 12px;}.sbe-mg-top {    margin-top: 20px}a.PreviewTooltip>.prefix {    margin: 0 !important;}',basis:THEMES.u},{name:"True Modern Dark Mode",description:"A true black and modern dark mode theme",css:/*$import*/"/* ==UserStyle==@ *name           skyblock.net@namespace      github.com/openstyles/stylus@version        1.0.0@description   True modern dark mode!@author         Athena/* =========================== *//* ===========DARK============ *//* =========================== */.dark .button {background: #040709 !important;}.dark .button.primary {background: #040709 !important;}.dark .useful-links > a {background: #04070991;border-color: #fff !important;}.dark #footer {background: #040709 !important;border-color: #fff !important;}.dark #content {background: #020405;}.dark #content .pageContent {background: #040709 !important;}.dark .sidebar .secondaryContent {background: #060a0c !important;}.dark .nodeList .categoryStrip {background: #020405 !important;}.dark .nodeList .categoryForumNodeInfo, .dark .nodeList .forumNodeInfo, .dark .nodeList .pageNodeInfo, .dark .nodeList .linkNodeInfo {background: #060a0c !important;border-color: #fff !important;}.dark .bedrock-alert {border: 1px solid #fff;color: #89f0f2;background: #5dd9db33;}.dark .nodeList .categoryStrip .nodeTitle a {color: #dff2ff !important;}.dark .node .nodeText .nodeTitle a {color: #dff2ff !important;}.dark .pairs dt, .dark .pairsInline dt, .dark .pairsRows dt, .dark .pairsColumns dt, .dark .pairsJustified dt {color: #799fbb !important;}.dark body {background: #060a0c;}.dark .sidebar .section .primaryContent h3 a, .dark .sidebar .section .secondaryContent h3 a {color: #fff !important;}.dark .sidebar .section .secondaryContent h3 {color: #fff !important;}.dark .secondaryContent a,.dark .secondaryContent .muted a {color: #fff !important;}.dark .node .nodeLastPost {background: #04070999 !important;border-color: #fff !important;}.dark .sidebar .section .secondaryContent .footnote, .dark .sidebar .section .secondaryContent .minorHeading {color: #fff !important;}.dark body .muted, .dark body a.muted, .dark body .muted a {color: #fff !important;}.dark .profilePostListItem , .dark .threadListItem {border-color: #fff !important;}.dark #footer .bottom {border-color: #ffffff1f !important;color: #fff;}.dark #footer .link-row > a {color: #fff;border-color: #fff !important;}.dark #footer .container .col h1 i {color: #bdd6e947;}.dark #footer .container .col h1 {color: #fff !important;}.dark #footer .social-row > a {background: #020405 !important;color: #fff !important;}.dark #footer .bottom a {background: #040709 !important;color: #fff !important;}.dark #footer .container .col p {color: #fff;}.dark .textCtrl {background: #040709 !important;border-color: #fff !important;}.dark .textCtrl::placeholder {color: #fff;}.dark .section-title h1 {color: #fff !important;}.dark .primaryContent {background: #060a0c !important;}.dark #content .sidebar .section {background: #2d4c62 !important;}.dark .recentNews .subHeading a.newsTitle {color: #fff !important;}.dark .recentNews .messageContent .postedBy {border-bottom: 1px solid #fff !important;}.dark #recentNews .recentNews .newsText {color: #fff;}.dark .primaryContent a {color: #fff;}.dark .recentNews .subHeading {border-bottom: 1px solid #fff !important;}.dark .recentNews .messageContent .postedBy {color: #fff;}.dark .card-date {color: #fff !important;background: #0f181d !important;}.dark .meta-text {color: #bdd6e9;}.dark .recentNews .subHeading .meta-text a:last-child {color: #fff !important;}.dark .discord-channel.discord-allusers-toggle {color: #dff2ff;}.dark .bbCodeBlock .attribution {background: #060a0c;border-color:#fff !important;}.dark .bbCodeBlock {border-color: #fff !important;background: #060a0c !important;}.dark .bbCodeQuote .quoteContainer {color: #dff2ff !important;}.dark #content .pageContent .button {background: #020405 !important;color: #fff !important;}.dark #content .pageContent .button.primary {color:#fff !important;background: #020405 !important;}.dark #content .bbCodeBlock .attribution .button {color:#dff2ff !important;}.attachedFiles .attachedFilesHeader {background: #020405;color: #fff !important;}.attachedFiles {background: #060a0c;}.dark #serverstatus .online {color: #a3cf70;}.dark .secondaryContent {background: #040709 !important;}.dark .Menu .sectionFooter {background: #040709 !important;}.dark .blockLinksList a, .dark .blockLinksList label {color: #dff2ff !important;}.dark .concealed, .dark .concealed a, .dark .cloaked, .dark .cloaked a {color: #dff2ff !important;}.dark .sectionFooter a {color: #fff !important;}.dark .titleBar {background: #060a0c !important;}.dark fieldset.breadcrumb {background: #060a0c !important;}.dark .sectionMain {background: #060a0c !important;}.dark #DiscussionListOptionsHandle a {background: #020405 !important;color: #fff !important;}.dark .titleBar h1 {color: #fff !important;}.dark .breadcrumb .crust a.crumb {color: #bdd6e9;}.dark .breadcrumb .crust .arrow span {border-left-color: #fff !important;}.dark .breadcrumb .crust .arrow {border-left-color: #fff !important;}.dark #pageDescription {color: #bdd6e9 !important;}.dark .discussionListItem {background: #040709 !important;border-color: #fff !important;}.dark .discussionList .title a {color: #dff2ff !important;}.dark a:link, .dark a:visited {color: #dff2ff;}.dark .textCtrl {color: #dff2ff !important;}.dark .sectionFooter {background: #04070999 !important;border-color: #fff !important;}.dark .formPopup .controlsWrapper {background: #040709 !important;}.dark .ctrlUnit > dd .explain {color: #dff2ff;}.dark .xenForm {background: #060a0c;}.dark .explain .CurrentStatus {color: #dff2ff;}.dark .xenForm fieldset, .dark .xenForm .formGroup {border-color: #fff !important;}.dark .ctrlUnit > dd > * > li .hint {color: #dff2ff;}.dark .ctrlUnit > dt dfn {color: #dff2ff;}.dark .xenForm fieldset + .ctrlUnit, .dark .xenForm .formGroup + .ctrlUnit, .dark .xenForm .submitUnit {border-color: #fff !important;}.discussionList .sticky .avatar img, .discussionList .sticky .avatar .img, .avatarCropper {border-color: #fff !important;}.dark .heading, .dark .xenForm .formHeader {color: #dff2ff !important;background: #24add3 !important;}.xenOverlay .formOverlay .heading {color: #dff2ff !important;}.dark .pageNavLinkGroup .PageNav {background: #060a0c;}.dark .messageList .message {background: #060a0c !important;}.dark .message .messageUserInfo {border-color: #fff !important;}.dark .PageNav span.pageNavHeader {color: #fff !important;}.dark .titleBar .below--title {border-color: #fff !important;}.dark #pageDescription a {color: #dff2ff !important;}.dark .PageNav a {background: #020405 !important;}.dark .secondaryContent {border-color: #040709 !important;}.dark .dark_postrating_outputlist {color: #dff2ff !important;}.dark .dark_postrating_outputlist li strong, .dark .dark_postrating_thread_rating strong {color: #dff2ff !important;}.dark .message .signature {color: #dff2ff !important;}.dark .message .signature {border-color: #fff !important;}.dark .quickReply.message {background: #040709 !important;}.dark .sectionHeaders {background: #060a0c !important;border-color: #fff !important;}.dark .discussionList .sectionHeaders a, .dark .discussionList .sectionHeaders a span {color: #fff !important;}.dark .discussionList .discussionListItem.sticky {background: #02040573 !important;border-color: #fff !important;}.dark .discussionList .sticky .pairsJustified dt {color: #fff !important;}.dark .discussionListItem .lastPostInfo .username {color: #fff !important;}.dark .sectionFooter {color: #fff !important;}.dark .discussionList .sticky .username {color: #ffffff !important;}.dark #headerMover #headerProxy:after {background: linear-gradient(0deg, #060a0c, transparent);}.dark div#topBar {background: #060a0c !important}#header #logo img {content:url(https://i.imgur.com/Tqv8Hiw.png)}.dark #headerMover #headerProxy {background-image: url(https://i.imgur.com/CDRkVpc.png)!important;}.dark div#landingHero {background: #020405 !important;}.dark div#navigation {border-color: #fff !important;background: #040709 !important;}.dark #loginBar {background: #5fc0da;}.dark #loginBar #loginBarHandle.userTabs {background: #5fc0da;}.dark .navTabs .publicTabs .navLink:before {color: #fff;}.dark .player-card {background: #fff !important;border-color: #fff !important;}.player-card i {color: #fff;}.dark #landingHero:before {background: #1a2e3c;}.dark body #headerMover #headerProxy:before {background: #1a2e3c;}.dark .pagenode_container #pageNodeContent {background: #040709 !important;}.dark #card-grid.mods .card {background: #020405 !important;}.dark .total-downloads {background: #020405;color: #bdd6e9;}.dark .total-downloads span {color: #fff;}.dark .card-text p {color: #bdd6e9;}.dark .card-text h1 {color: #dff2ff !important;}.dark .page article p font {color: #bdd6e9 !important;}.dark .pagenode_container #pageNodeContent.page article > center font b {color: #dff2ff;}.dark .vote-flex .vote-links > a {background: #020405 !important;}.dark .vote-flex .vote-links > a .vote-link-text-large {color: #dff2ff !important;}.dark .card {background: #020405 !important;}.dark .vote-desc {color: #fff !important;}.dark .play-wrapper .install-text a {background: #020405 !important;}.dark .faqs-item {background: #020405 !important;color: #fff;}.dark .page article p font b {color: #dff2ff;}.dark .faqs-title {color: #dff2ff !important;}.dark .step {background: #ffffff1f !important;color: #dff2ff !important;}.dark .install-text {background: #020405 !important;color: #fff;}.dark .install-text b {color: #fff;}.dark .install-text h1 {color: #fff;}.dark .pagenode_container #pageNodeContent.page article > font b {color: #dff2ff;}.dark .vote-flex .vote-links > a .vote-link-text-small {color: #dff2ff !important;}.dark .vote-flex .vote-link-icon {color: #dff2ff !important;}.dark .vb-item {color: #bdd6e9 !important;}#XenForo {background: #060a0c;}.dark .redactor_toolbar {background: #020405;}.dark .bbCodeQuote .quoteContainer .quoteExpand {background: linear-gradient(0deg, #000, transparent);}.dark .Popup .PopupControl:hover, .dark .Popup.PopupContainerControl:hover {color: #fff !important;}.dark .secondaryContent {background: #040709 !important;}.dark .Menu .primaryContent {background: #040709 !important;color: #fff !important;}.dark div#exposeMask {background: #020405 !important;}.dark .xenOverlay.memberCard .userInfo h3 a {color: #fff !important;}.xenOverlay.memberCard .userInfo h4 {color: #fff;}.dark .xenOverlay.memberCard {background: #060a0c !important;}.dark .xenOverlay.memberCard .userLinks {color: #fff;border-color: #fff;}.dark .xenOverlay.memberCard .lastActivity {border-color: #fff;}.dark .pairs dt, .dark .pairsInline dt, .dark .pairsRows dt, .dark .pairsColumns dt, .dark .pairsJustified dt {color: #fff !important;}.dark .pairsJustified dd {color: #dff2ff !important;}.InlineModChecked .messageUserBlock, .InlineModChecked .messageInfo, .InlineModChecked .messageNotices, .InlineModChecked .bbCodeBlock .type, .InlineModChecked .bbCodeBlock blockquote, .InlineModChecked .attachedFiles .attachedFilesHeader, .InlineModChecked .attachedFiles .attachmentList {background: #4e6d81;}.profilePage .primaryUserBlock h1 {color: #fff !important;}.tabs li a, .tabs.noLinks li {background: transparent !important;}.dark .profilePage .mainProfileColumn {background: #040709 !important;}.dark .profilePage .mainProfileColumn .primaryContent, .dark .profilePage .mainProfileColumn .secondaryContent {border-color: #fff !important;}.dark .profilePage .tabs.mainTabs {border-top: 1px solid #fff !important;}.dark .profilePage .profilePoster textarea {background: #040709 !important;}.xenOverlay .section .heading {color: #dff2ff !important;}.memberListItem {border-color: #ffffff29 !important;}.xenOverlay .sectionFooter .button, .xenOverlay .sectionFooter .buttonContainer, .xenForm .button {background: #040709 !important;color: #fff !important;}.discussionList .discussionListItem.deleted .deletionNote {color: #ffdd76 !important;}.pairsInline dd {color: #fff !important;}.xenOverlay .sectionFooter .button.primary, .xenForm .button.primary {color: black !important;background: #fff !important;}.discussionListItem.InlineModChecked, .discussionListItem.InlineModChecked .posterAvatar, .discussionListItem.InlineModChecked .main, .discussionListItem.InlineModChecked .stats, .discussionListItem.InlineModChecked .lastPost, .discussionListItem.deleted.InlineModChecked, .discussionListItem.moderated.InlineModChecked {background: #ffffff0a !important;}.NewsFeedItem:not(:first-child) {border-top: 1px solid #ffffff29;}.dark .xenForm .ctrlUnit > dd .textCtrl {background: #020405 !important;}.dark .bbCodeBlock .type {color: #fff !important;}.dark #landingHero .player-card {color: #fff !important;}.dark #SignupButton .inner {background: #020405 !important;color: #fff !important;}.dark #mobileMenu {background: #040709fa !important;}#loginBar .xenForm .ctrlUnit, #loginBar .xenForm .ctrlUnit > dt label {color: #fff !important;}.Menu {background-color: #040709; border: 1px solid #fff; border-top-color: #fff; border-top: 5px solid #fff;}.copy-text {color: #fff;}#calroot {background-color: #040709; border: 1px solid #fff;}#caltitle {color: #fff;}#caldays span {color: #fff;}#caldays {border-bottom: 1px solid #fff;}#calcurrent {background-color: #fff; color: #000;}a.caloff:hover {background-color: #fff; color: #000}.calweek a:hover, .calfocus {background-color: #fff;color: #000}.profilePage .primaryUserBlock {border-top: 1px solid #fff;}ul.autoCompleteList li:hover,ul.autoCompleteList li.selected {background-color: #fff; color: #000;}ul.autoCompleteList {background-color: #040709; border: 1px solid #fff;}.total-downloads.hti {background: #020405 !important;}.avatar img,.avatar .img,.avatarCropper{background-color: #020405; border: 1px solid #fff;}.xenPreviewTooltip{border: 10px solid #fff; border-color:  #fff;}.xenPreviewTooltip .arrow{border-top:  15px solid #fff; border-bottom: 1px none #fff;}.xenPreviewTooltip .arrow span {border-top: 15px solid #fff; border-bottom: 1px none #fff;}.attachedFiles{border: 1px solid #fff;}.attachment .boxModelFixer{background-color: #020405; border: 1px solid #fff;}.attachment .thumbnail{border-right: 1px solid #fff;}.attachedFiles .attachedFilesHeader{color: #fff; background: #020405 repeat-x top; border-bottom: 1px solid #fff;}.redactor_box {border: 1px solid #fff;}html {background-color: #020405; color: rgb(255, 255, 255);}.redactor_toolbar {background-color: #020405; border-bottom: 1px solid #fff;}.profilePage .primaryUserBlock .lastActivity{border-top: 1px solid #fff;}.profilePage .mast{border-right: 1px solid #fff;}.subHeading{background-color: #020405; border-top: 1px solid #fff; border-bottom: 1px solid #fff;}.dark_postrating_member {border-color: #fff; color: rgb(255, 255, 255);}.dark_postrating_member td {border-color: #fff;}.profilePage .mainProfileColumn {border-left: 1px solid #fff;}.profilePage .mast .followBlocks .primaryContent.avatarHeap {border: 1px solid #fff;border-top-color: #fff;}.textWithCount.subHeading .count {border: 1px solid #fff; background-color: #020405; color: rgb(255, 255, 255);}.bbCodeQuote .quoteContainer .quoteExpand {color: #fff; background: linear-gradient(to bottom, rgba(83, 123, 147, 0) 0%, rgb(83, 123, 147) 80%);}.tabs {background-color: #020405; border-bottom: 1px solid #fff;}.friend {background-color: #020405;}.dataTable tr.dataRow th {background: #020405 repeat-x top; border-bottom: 1px solid #fff; border-top: 1px solid #fff; color: rgb(255, 255, 255);}.dataTable tr.dataRow td {border-bottom: 1px solid #fff;}.textHeading, .xenForm .sectionHeader {color: rgb(255, 255, 255); border-bottom: 1px solid #fff;}.searchResult {border-bottom: 1px solid #fff;}.staffTitle {background: #020405 repeat-x top; border-bottom: 1px solid #fff;}.memberListItem .extra .bigNumber {color: #fff;}body .dimmed, body a.dimmed, body .dimmed a {color: #fff;}a.callToAction {background: #020405 !important;}.bbCodeBlock pre, .bbCodeBlock .code{background: #060a0c repeat-x top;}.bbCodeBlock .type {border-bottom: 1px solid #fff;}",basis:THEMES.k}];if(ls.getItem("customThemes")){const U=JSON.parse(ls.getItem("customThemes")??"[]");0<U.length&&(U.map(o=>{null!=o.v&&(o.basis=o.v?THEMES.p:THEMES.u,delete o.v),o.C=()=>addStyle(o.css??"")}),themes=[...themes,...U])}if("true"==localStorage.getItem("customThemeMode")&&localStorage.getItem("customTheme-SBE")){const W=JSON.parse(localStorage.getItem("customTheme-SBE")??"[]");W.css?addStyle(W.css??""):W.C&&W.C(),document.querySelector('.pageContent>.choosers>dd>a[href*="misc/style?redirect"]').innerHTML=W.name}if(waitForElm(".section.styleChooser").then(o=>{console.log("Style chooser opened!"),document.querySelectorAll('a[href^="misc/style"]').forEach(o=>{const e=o;e.addEventListener("click",function(o){var t=e.href.match(/style_id=(\d+)/)[1];localStorage.setItem("sbe-vanilla-style",t.toString())})}),console.log("Overlay found!");var t=o.querySelector("ol.twoColumns.primaryContent.chooserColumns");for(const n of AF(t.querySelectorAll("li"))){const i=n.querySelector("a");/style_id=(1|6|22|30|31|16)/.test(i?.href??"")&&i?.addEventListener("click",o=>{o.preventDefault(),localStorage.setItem("customThemeMode","false"),window.location.href=i?.href})}for(const s of themes){var e=t.querySelector("li")?.cloneNode(!0),r=e.querySelector("a"),r=(r?.removeAttribute("href"),r?.addEventListener("click",o=>{o.preventDefault(),localStorage.setItem("customThemeMode","true"),localStorage.setItem("customTheme-SBE",JSON.stringify(s)),window.location.href=`https://skyblock.net/misc/style?style_id=${s.basis}&_xfToken=${xfToken}&redirect=`+encodeURI(window.location.href)}),e.querySelector(".title")),a=e.querySelector(".description");r.innerHTML=s.name,a.innerHTML=s.description,e.classList.add("sbe-pointer"),t.appendChild(e)}}),isOnNewTheme&&!document.querySelector(".changeTheme:not(.sbe-change-theme)")){const la=document.querySelector("div#footer>div.bottom>div.container");la.innerHTML+=`<a href="misc/style?redirect=${encodeURIComponent(location.pathname)}" class="changeTheme OverlayTrigger Tooltip sbe-change-theme" title="Style Chooser" rel="nofollow">Change Theme</a>`}if(console.log("skyblock extras loaded"),settings.threadTitleEnabled){const ma=document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]');ma?.textContent&&(document.title=ma.textContent+" | Skyblock Forums")}if(settings.adBlocker&&document.querySelectorAll(".adsbygoogle").forEach(o=>{o.remove()}),settings.hideShopTab){const oa=document.querySelector("ul.publicTabs"),pa=oa?.children[7],qa=oa?.children[6];pa&&qa?.replaceWith(pa)}if(settings.strikethroughBannedUsers){const ra=document.querySelectorAll(".messageUserBlock"),sa=Array.from(ra).filter(o=>o.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]'));sa.forEach(o=>{o.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.SBonlIntegration&&isOnUserProfile){const va=document.querySelector('[href="misc/quick-navigation-menu"]'),wa=va?.cloneNode(!0),xa="http://skyblock.onl/@"+presentUserProfile[1];wa.href=xa,wa.style.background="url('data:image/png;base64,')",va?.parentElement?.appendChild(wa)}if(settings.actualDateOnFrontpage){
//@ts-ignore
const ya=o=>strftime("%H:%M %d/%m/%Y",o??null),za=document.querySelectorAll(".dateTime .DateTime");za.forEach(o=>{var t=o.getAttribute("data-time")??null;null!==t&&(o.innerHTML=ya(new Date(1e3*parseInt(t))))}),patchClass(XenForo.TimestampRefresh?.prototype,"refresh",(o,t,e)=>{})}if(settings.fixBedrockPlayersImages&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(o=>{o.addEventListener("click",o=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(o=>{o.getAttribute("src")?.startsWith("MHF_")||o.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.responsiveModals&&!async function(){await waitForElm(".memberCard");let r;setInterval(()=>{var o,t,e=document.body.clientWidth;between(e,e-10,e+10)&&(t=(e/2-((o=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",o.style.left=t,r=e)},500)}(),settings.movePoke&&isOnUserProfile){const Na=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),Oa=Na?.querySelector('li > a.OverlayTrigger[href^="pokes/"]');
// the better option was apparently to delete the ability to poke on profiles on the old theme and fix it on the new one instead of fixing it on both
if(Na&&Oa){const Pa=Oa?.cloneNode(!0);// remove the dropdown if there's only the one element in the dropdown
(1==Na?.childElementCount?Oa?.parentElement:Oa)?.remove();//otherwise only remove the poke option
let o=document.createElement("li");o.appendChild(Pa),document.querySelector(".followBlock > ul")?.appendChild(o),0===Na?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}}if(settings.ratingRatio&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const Ra=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),Sa=Ra?.querySelector("tbody");Ra.style.padding="5px 20px",Array.from(Sa?.children??[]).forEach(o=>{var t,e,r;o.querySelector("th")?((t=document.createElement("th")).innerHTML="Ratio:",o.appendChild(t)):(t=o.children[1],e=o.children[2],(r=document.createElement("td")).setAttribute("class",e.getAttribute("class")??"dark_postrating_neutral"),r.innerHTML=calculateRatio(parseInt(t.innerHTML.replace(/,/g,"")),parseInt(e.innerHTML.replace(/,/g,""))),o.appendChild(r))})}if(settings.removeRatingCommas&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(o=>{o.innerHTML.replace(/,/g,"")}),settings.avatarOnProfileStats&&isOnUserProfile){let o=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');o.style.position="relative",o.children[0].style.position="absolute"}if(settings.birthdayHatOnPFP){
// https://www.flaticon.com/free-icon/party-hat_2668968
const $a="data:image/png;base64,",_a=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(o=>{var t,e,r;"Birthday:"===o.children[0].innerHTML&&(t=(o=colonNumber(o.children[1].innerHTML.split(" "),2))[0],o=parseInt(o[1]??1),t=getMonthFromString(t),[e,r]=[(e=new Date).getMonth()+1,e.getDate()],o===r)&&t===e&&(_a.style.position="relative",(o=document.createElement("img")).src=$a,o.width=256,o.height=256,o.style.width="256px",o.style.height="256px",o.style.position="absolute",o.style.zIndex="999",o.style.left="100px",o.style.top="-420px",o.style.rotate="30deg",o.style.pointerEvents="none",_a.appendChild(o))})}if(settings.roundedFriendsOnProfile&&isOnUserProfile&&addStyle(".friend>.friend-head>img { border-radius: 7px; };"),settings.postLinkButton&&isOnThread&&(AF(document.querySelectorAll('li[id^="post-"].message[data-author]')).forEach(o=>{var t=o.id.split("-")[1],o=o.querySelector(".publicControls");const e=document.createElement("a");e.href="https://skyblock.net/posts/"+t,e.setAttribute("class","ReplyQuote item control reply sbe-msg-link"),e.title="Copy link to this message.",e.innerHTML="<span></span>Copy Link",e.onclick=()=>window.navigator.clipboard.writeText(e.href),o?.appendChild(e)}),
// Fix copy link/copy bbcode showing up in the selected text hover thing
patchClass(XenForo.SelectQuotable?.prototype,"createButton",function(o,t){var o=o(),e=this.$button[0].children;for(const r of Array.from(e))r.classList.contains("sbe-msg-link")&&r.remove();return o})),settings.postLinkButton&&isOnUserProfile&&AF(document.querySelectorAll('li[id^="profile-post-"].messageSimple[data-author]>.messageInfo>.messageMeta>.publicControls')).forEach(o=>{
/* self-comments won't have like, but for ones with like we still want to insert before like */
var o=o.querySelector("a.LikeLink.like")??o.querySelector("a.CommentPoster.postComment"),t=o.href.replace("https://skyblock.net/","").split("/")[1];console.log(o,t);const e=document.createElement("a");e.classList.add("item","control","copylink","sbe-msg-link"),e.href="profile-posts/"+t,e.onclick=()=>navigator.clipboard.writeText(e.href);t=document.createElement("span");t.innerHTML="Copy Link",e.appendChild(t),o.insertAdjacentElement("beforebegin",e)}),settings.minotarNotCrafatar&&setTimeout(()=>{AF(document.querySelectorAll('img[src^="https://crafatar.com/avatars"]')).forEach(o=>{console.log(o),o.setAttribute("src",(o.getAttribute("src")??"").replace(/https\:\/\/crafatar.com\/avatars\/([a-fA-F0-9\-]{32,36})\?size=(\d+)&overlay=true/g,"https://minotar.net/avatar/$1/$2"))})},1e3),settings.noMoreCamo&&setTimeout(()=>{AF(document.querySelectorAll('[src*="camo.skyblock.net/"]')).forEach(o=>{var t=decodeURIComponent(o.getAttribute("src")??"").split("?url"),t=decodeURIComponent(2==t.length?t[1]:t[2]),t=t.startsWith("=")?t.slice(1):t;o.setAttribute("src",t)})},1e3),settings.fadeInReactions&&addStyle(".dark_postrating_inputlist {transition: opacity 0.5s;}.dark_postrating_inputlist:not(:hover) {opacity: 0.1 !important;}"),settings.moreSearchOnCard&&(patchClass(XenForo.OverlayLoader?.prototype,"createOverlay",(o,t)=>{var e;return t.templateHtml?(t.templateHtml.includes('<div id="memberCard')&&(e=`<dt>Threads: </dt><dd><a href="search/member?user_id=${t.templateHtml.match(/<a href="members\/.+\.([0-9]+)\/"\>Profile Page<\/a>/)?.[1]??"1"}&content=thread" class="concealed" rel="nofollow">Search</a></dd>`,t.templateHtml=insert(t.templateHtml,t.templateHtml.indexOf("\x3c!-- slot: pre_likes"),e)),settings.imgurProxyEnabled&&t.templateHtml.includes('src="https://i.imgur.com')&&(console.log("replacing imgur url(s) in overlay template html"),t.templateHtml=t.templateHtml.replace(/src=(["'])https:\/\/i\.imgur\.com/g,"src=$1"+(settings.rimgoInstanceHost.match(/^https?:\/\//)?settings.rimgoInstanceHost:"https://"+settings.rimgoInstanceHost)))):console.log("createoverlay does not contain templatehtml: "+t),o(t)}),isOnUserCard)){const I0=document.querySelector(".userInfo"),J0=I0.querySelector('.userLinks>a[href$="/"][href^="members/"]').href.split("/").at(-2)?.split(".")[1],K0=I0.querySelector(".userStats"),L0=document.createElement("dt"),M0=(L0.innerHTML="Threads: ",document.createElement("dd")),N0=document.createElement("a");N0.href=`search/member?user_id=${J0}&content=thread`,N0.classList.add("concealed"),N0.rel="nofollow",N0.innerHTML="Search",M0.appendChild(N0),K0?.insertBefore(M0,K0.childNodes[11]),K0?.insertBefore(L0,K0.childNodes[11])}settings.unpinLawsuit&&isOnIndex&&(document.querySelector('[id="recentNews"]>[id="145369"]')?.remove(),document.querySelector('[id="recentNews"]>[id="147906"]')?.remove()),settings.fixOldLinks&&document.querySelectorAll('a[href*="block.net"]').forEach(o=>{console.log;o.href=o.href
/* replace a http(s) (or no protocol) url with, or without www. on endblock.net, hellblock.net or skyblock.net to be https://skyblock.net */.replace(/^((?:https?:\/\/)?(?:w{3})?\.(?:end|hell|sky)block\.net)/g,"https://skyblock.net").replaceAll("skyblock.net/index.php?threads","skyblock.net/threads")}),settings.dontShare&&addStyle("div.sharePage {display: none;}"),settings.copyMessageBBCodeButton&&isOnThread&&[...AF(document.querySelectorAll('li[id^="post-"].message[data-author]')),...AF(document.querySelectorAll('li[id^="message-"].message[data-author]'))].forEach(o=>{const e=o.id.split("-")[1];var o=o.querySelector(".publicControls"),t=document.createElement("a");t.href="#",t.setAttribute("class","ReplyQuote item control reply sbe-msg-link"),t.title="Copy message BBCode.",t.innerHTML="<span></span>Copy BBCode",t.onclick=async o=>{o.preventDefault();var o=await(await fetch(`https://skyblock.net/posts/${e}/quote`,{headers:{I:"application/x-www-form-urlencoded; charset=UTF-8"},body:"_xfResponseType=json&_xfToken="+xfToken,method:"POST",credentials:"include"})).json(),t=o.quote?o.quote.trim().match(/^\[QUOTE="[^,]+, post: \d+, member: \d+"\](.+)\[\/QUOTE]$/s):"";if(!o.quote||!t)return console.log("body.quote",o.quote),console.log("quoteunquoted",t),window.navigator.clipboard.writeText("<failed to get content>");window.navigator.clipboard.writeText(t[1])},o?.appendChild(t)}),settings.imgurProxyEnabled&&AF(document.querySelectorAll('img[src^="https://i.imgur.com"]')).forEach(o=>{o.src=o.src.replace("https://i.imgur.com",settings.rimgoInstanceHost.match(/^https?:\/\//)?settings.rimgoInstanceHost:"https://"+settings.rimgoInstanceHost)});
