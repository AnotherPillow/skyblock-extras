// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @match       https://skyblock.net/*
// @grant       none
// @version     1.0
// @author      AnotherPillow
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// @description A userscript to improve the skyblock.net forums experience!
// ==/UserScript==
"use strict";function waitForElm(n){return new Promise(t=>{if(document.querySelector(n))return t(document.querySelector(n));const e=new MutationObserver(A=>{document.querySelector(n)&&(e.disconnect(),t(document.querySelector(n)))});e.observe(document.body,{childList:!0,subtree:!0})})}var _listenForChildUpdateObservers={};function listenForChildUpdate(A,t){var e=document.querySelector(A);
//@ts-ignore
_listenForChildUpdateObservers[A]=new MutationObserver(function(A){A.forEach(function(A){t(e)})});
// pass in the target node, as well as the observer options
//@ts-ignore
_listenForChildUpdateObservers[A].observe(e,{attributes:!0,childList:!0,characterData:!0})}function between(A,t,e){return t<=A&&A<=e}const isOnUserProfile=window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1;
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(A,t){for(var e=t;1<e;e--)A%e==0&&t%e==0&&(A/=e,t/=e);return A+":"+t}const AF=Array.from,colonNumber=(A,e)=>A.filter((A,t)=>t<e).filter(A=>""!==A),getMonthFromString=A=>new Date(Date.parse(A+" 1, 2012")).getMonth()+1;class _Settings{A=!0;t=!0;o=!0;i=!0;g=!1;v=!0;m=!0;l=!0;B=!0;C=!0;I=!0;P=!0;u=!1;p=!0;h;k(A,t){var e="sbe-setting-"+t.toString().replace(/\s/g,"_"),n=document.createElement("label"),A=(n.innerHTML=A+": ",n.setAttribute("for",e),document.createElement("input"));A.name=e,A.id=e,A.type="checkbox",
//@ts-ignore
A.checked=this[t]??!1,A.addEventListener("click",A=>{A=//@ts-ignore
A.target.checked;this[t]=A}),this.h?.appendChild(n),this.h?.appendChild(A),this.H()}H(){this.h?.appendChild(document.createElement("br"))}constructor(){this.S(),this.h=document.createElement("dialog");var A=document.createElement("h1"),A=(A.innerHTML="Skyblock Extras Settings",A.style.fontSize="2em",this.h.appendChild(A),this.H(),this.h.style.width="640px",this.h.style.height="480px",this.h.style.borderRadius="1em",this.h.style.border="none",this.h.style.outline="none",this.h.style.textAlign="center",this.k("Thread Title as Browser Title","threadTitleEnabled"),this.k("Remove the shop tab","hideShopTab"),this.k("Strike through banned users' names","strikethroughBannedUsers"),this.k("Better New Style Theme","betterNewSB"),this.k("Update Font Awesome","fontAwesomeUpdate"),this.k("Skyblock.onl Integration","SBonlIntegration"),this.k("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.k("Fix bedrock players' images","fixBedrockPlayersImages"),this.k("Responsive Modals","responsiveModals"),this.k("Move poking out of moderator tools","movePoke"),this.k("Add given:received ratio for reactions on profile","ratingRatio"),this.k("Remove commas from ratings","removeRatingCommas"),this.k("Avatar on profile stats.","avatarOnProfileStats"),this.k("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),document.createElement("button")),A=(A.innerHTML="Save",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.J(),window.location.reload()}),this.H(),this.H(),this.h.appendChild(A),document.createElement("button")),A=(A.innerHTML="Close",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.close()}),this.H(),this.H(),this.H(),this.H(),this.h.appendChild(A),document.body.appendChild(this.h),document.createElement("img"));A.src=" data:image/png;base64,",A.id="sbe-settings-opener-img",A.style.marginLeft="9em",A.style.marginTop="2px",A.height=20,A.width=20,A.addEventListener("click",A=>{this.open()}),document.querySelector('[class="navTabs"]')?.insertBefore(A,document.querySelector(".visitorTabs"))}open(){this.h?.showModal()}close(){this.h?.close()}J(){localStorage.setItem("sbe-settings",JSON.stringify({A:this.A,t:this.t,o:this.o,i:this.i,g:this.g,v:this.v,m:this.m,l:this.l,B:this.B,C:this.C,I:this.I,P:this.P,u:this.u,p:this.p}))}S(){var A=JSON.parse(localStorage.getItem("sbe-settings")??"{}");for(const t of Object.keys(A))
//@ts-ignore
this[t]=A[t]}}const settings=new _Settings;if(settings.g){const O="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.2.1/css/fontawesome.min.css",P=document.querySelector('link[href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css"]'),Q=(P.disabled=!0,console.log(P),P?.remove(),document.createElement("link")),R=(Q.rel="stylesheet",Q.href=O,document.head.appendChild(Q),document.querySelectorAll('[class*="fa"'));R.forEach(A=>{A.classList.remove("fa"),A.classList.add("fa-solid"),A.classList.contains("fa-heart")&&(A.outerHTML="❤️")})}if(document.querySelector(".navTabs")){const T=`
        .navTabs {
            position: relative;
        }

        .navTabs::before {
            content: "+";
            position: absolute;
            z-index: 20;
            color: white;
            top: -0.7rem;
            left: 5px;
            font-size: 3em;
        }
    `;GM_addStyle(T)}if(settings.A&&(document.title=(document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]'))?.textContent+" | Skyblock Forums"),settings.t){const U=document.querySelector("ul.publicTabs"),V=U?.children[7],W=U?.children[6];V&&W?.replaceWith(V)}if(settings.o){const X=document.querySelectorAll(".messageUserBlock"),Y=Array.from(X).filter(A=>A.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]')),Z=`
        .sbe-strikethrough {
            text-decoration: line-through !important;
            text-decoration-thickness: 2px !important;
        }
    `;GM_addStyle(Z),Y.forEach(A=>{A.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.i){const aa=`
    div.navTabs {
        background:#2b485c;
        border-radius: 0 !important;
    }
    #landingHero>* {
        display:none !important;
        background: none !important;
    }
    #landingHero {
        height:25px;
        padding:0 !important;
    }
    #content .sidebar .section {
        border-radius: 0px !important;
    }
    #content .section {
        -webkit-box-shadow:none !important;
        box-shadow:none !important;
    }
    .avatar img, .avatarWrap .img.s {
        border-radius: 5px !important;
    }
    .visitorTabs, .navTabs .visitorTabs {
        display: block !important;    
    }
    div#navigation {
        border-bottom: none;
    }
    #content .sidebar .section .secondaryContent {
        padding: 15px !important;
    }
    #footer>.top {
        padding:25px;
    }
    #content {
        background: #d1eef5 !important;
    }
    .newsText {
        color: #113240
    }
    li[id^="thread"]>.title {
        font-size: 12px;
    }
    .sbe-mg-top {
        margin-top: 20px
    }

    a.PreviewTooltip>.prefix {
        margin: 0 !important;
    }
    `;document.querySelector('[data-clipboard-text="play.skyblock.net"]')&&GM_addStyle(aa)}if(settings.v&&isOnUserProfile){const ba=document.querySelector('[href="misc/quick-navigation-menu"]'),ca=ba?.cloneNode(!0);ca.href="http://skyblock.onl/@"+isOnUserProfile[1],ca.style.background="url('data:image/png;base64,')",ba?.parentElement?.appendChild(ca)}if(settings.m){
//@ts-ignore
const da=A=>strftime("%H:%M %d/%m/%Y",A??null),ea=document.querySelectorAll(".dateTime .DateTime");ea.forEach(A=>{var t=A.getAttribute("data-time")??null;null!==t&&(A.innerHTML=da(new Date(1e3*parseInt(t))))})}if(settings.l&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(A=>{A.addEventListener("click",A=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(A=>{A.getAttribute("src")?.startsWith("MHF_")||A.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.B&&!async function(){await waitForElm(".memberCard");let n;setInterval(()=>{var A,t,e=document.body.clientWidth;between(e,e-10,e+10)&&(t=(e/2-((A=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",console.log(A),A.style.left=t,n=e)},500)}(),settings.C&&isOnUserProfile){const pa=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),qa=pa?.querySelector('li > a.OverlayTrigger[href^="pokes/"]'),ra=qa?.cloneNode(!0);qa?.parentElement?.remove();let A=document.createElement("li");A.appendChild(ra),document.querySelector(".followBlock > ul")?.appendChild(A),0===pa?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}if(settings.I&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const ta=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),ua=ta?.querySelector("tbody");ta.style.padding="5px 20px",console.log(ua),Array.from(ua?.children??[]).forEach(A=>{var t,e,n;console.log(A),A.querySelector("th")?((t=document.createElement("th")).innerHTML="Ratio:",A.appendChild(t)):(t=A.children[1],e=A.children[2],console.log(t,e),(n=document.createElement("td")).setAttribute("class",e.getAttribute("class")??"dark_postrating_neutral"),n.innerHTML=calculateRatio(parseInt(t.innerHTML.replace(/,/g,"")),parseInt(e.innerHTML.replace(/,/g,""))),A.appendChild(n))})}if(settings.P&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(A=>{A.innerHTML.replace(/,/g,"")}),settings.u&&isOnUserProfile){let A=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');A.style.position="relative",A.children[0].style.position="absolute"}if(settings.p){
// https://www.flaticon.com/free-icon/party-hat_2668968
const Ca="data:image/png;base64,",Da=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(A=>{var t,e,n;"Birthday:"===A.children[0].innerHTML&&(t=(A=colonNumber(A.children[1].innerHTML.split(" "),2))[0],A=parseInt(A[1]??1),t=getMonthFromString(t),[e,n]=[(e=new Date).getMonth()+1,e.getDate()],A===n)&&t===e&&(Da.style.position="relative",(A=document.createElement("img")).src=Ca,A.width=256,A.height=256,A.style.width="256px",A.style.height="256px",A.style.position="absolute",A.style.zIndex="999",A.style.left="100px",A.style.top="-420px",A.style.rotate="30deg",A.style.pointerEvents="none",Da.appendChild(A))})}
