// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @description A userscript to improve the skyblock.net forums experience!
// @match       https://skyblock.net/*
// @grant       none
// @version     1.2.2
// @author      AnotherPillow
// @license     GNU GPLv3
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// @downloadURL https://anotherpillow.github.io/skyblock-extras/build/sbe.min.js
// ==/UserScript==
"use strict";window.t=XenForo||window.t||{},globalThis.t=XenForo||globalThis.t||window.t||{};let SELECTED_VANILLA_THEME=localStorage.getItem("sbe-vanilla-style");function waitForElm(n){return new Promise(t=>{if(document.querySelector(n))return t(document.querySelector(n));const o=new MutationObserver(e=>{document.querySelector(n)&&(o.disconnect(),t(document.querySelector(n)))});o.observe(document.body,{childList:!0,subtree:!0})})}function between(e,t,o){return t<=e&&e<=o}
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(e,t){for(var o=t;1<o;o--)e%o==0&&t%o==0&&(e/=o,t/=o);return e+":"+t}const AF=Array.from,colonNumber=(e,o)=>e.filter((e,t)=>t<o).filter(e=>""!==e),getMonthFromString=e=>new Date(Date.parse(e+" 1, 2012")).getMonth()+1,getHrefWithoutAnchor=()=>window.location.href.replace(new RegExp(window.location.hash+"$"),""),isOnThread=getHrefWithoutAnchor().match(/https\:\/\/skyblock\.net\/threads\/.+\.\d+\/?/),isInConversation=getHrefWithoutAnchor().match(/https\:\/\/skyblock\.net\/conversations\/.+\.\d+\/?/),isOnUserProfile=window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1,isOnIndex="https://skyblock.net/"==window.location.href,isOnOriginalTheme=!document.querySelector('.social-row>[href="https://www.reddit.com/r/SkyBlock"]')&&document.querySelector('.pageContent>span>a[href="http://blackcaffeine.com/"]'),isOnMiddleTheme=!document.querySelector('a[href="http://blackcaffeine.com/"]')&&!document.querySelector('.social-row>[href="https://www.reddit.com/r/SkyBlock"]'),isOnNewTheme=document.querySelector('.link-row>a[href="/how-to-install-skyblock"]')&&document.querySelector("#footer>.top>.container>.col>p"),$import=e=>(alert("If you are seeing this, something has gone very wrong."),""),xfToken=XenForo._csrfToken,ls=localStorage;function patchClass(e,t,o){if(void 0===e)return console.log(`Cannot patch ${t} of ${e}.`);const n=e[t];console.log("Patching "+t,n),e[t]=function(...e){return o.call(this,n.bind(this),...e)}}
// adapted from https://stackoverflow.com/a/9160869
GM_addStyle(/*$import*/'.sbe-pointer {    cursor: pointer;}#sbe-settings-modal {    width: 640px;    height: 560px;    border-radius: 1em;    border: medium;    outline: none;    text-align: center;    scrollbar-width: none;}.navTabs {    position: relative;}.navTabs::before {    content: "+";    position: absolute;    z-index: 20;    color: white;    top: -0.7rem;    left: 5px;    font-size: 3em;}.sbe-strikethrough {    text-decoration: line-through !important;    text-decoration-thickness: 2px !important;}.message .messageMeta {    width: 95%;}');const insert=(e,t,o)=>0<t?e.substring(0,t)+o+e.substring(t,e.length):o+e;class _Settings{threadTitleEnabled=!0;hideShopTab=!0;strikethroughBannedUsers=!0;SBonlIntegration=!0;actualDateOnFrontpage=!0;fixBedrockPlayersImages=!0;responsiveModals=!0;movePoke=!0;ratingRatio=!0;removeRatingCommas=!0;avatarOnProfileStats=!1;birthdayHatOnPFP=!0;roundedFriendsOnProfile=!0;postLinkButton=!0;minotarNotCrafatar=!0;noMoreCamo=!1;fadeInReactions=!0;adBlocker=!0;moreSearchOnCard=!0;unpinLawsuit=!0;fixOldLinks=!0;dontShare=!0;o=!0;i;A(e,t){var o="sbe-setting-"+t.toString().replace(/\s/g,"_"),n=document.createElement("label"),e=(n.innerHTML=e+": ",n.setAttribute("for",o),document.createElement("input"));e.name=o,e.id=o,e.type="checkbox",e.checked=this[t]??!1,e.addEventListener("click",e=>{e=//@ts-ignore
e.target.checked;this[t]=e}),this.i?.appendChild(n),this.i?.appendChild(e),this.l()}l(){this.i?.appendChild(document.createElement("br"))}constructor(){this.g(),this.i=document.createElement("dialog"),this.i.id="sbe-settings-modal";var e=document.createElement("h1"),e=(e.innerHTML="Skyblock Extras Settings",e.style.fontSize="2em",this.i.appendChild(e),this.l(),this.A("Thread Title as Browser Title","threadTitleEnabled"),this.A("Remove the shop tab","hideShopTab"),this.A("Strike through banned users' names","strikethroughBannedUsers"),this.A("Skyblock.onl Integration","SBonlIntegration"),this.A("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.A("Fix bedrock players' images","fixBedrockPlayersImages"),this.A("Responsive Modals","responsiveModals"),this.A("Move poking out of moderator tools","movePoke"),this.A("Add given:received ratio for reactions on profile","ratingRatio"),this.A("Remove commas from ratings","removeRatingCommas"),this.A("Avatar on profile stats","avatarOnProfileStats"),this.A("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),this.A("Round friends' icons on profile","roundedFriendsOnProfile"),this.A("Add button to copy link to post on posts","postLinkButton"),this.A("Replace Craftar with Minotar","minotarNotCrafatar"),this.A("Remove Skyblock's image proxy","noMoreCamo"),this.A("Fade in reaction opacity on hover","fadeInReactions"),this.A("Block Ads","adBlocker"),this.A("More search options on member card","moreSearchOnCard"),this.A("Unpin Lawsuit","unpinLawsuit"),this.A("Fix old forum links","fixOldLinks"),this.A("Remove share buttons","dontShare"),this.A("Add a button to copy message BBCode","copyMessageBBCodeButton"),document.createElement("button")),e=(e.innerHTML="Save",e.style.width="6em",e.style.height="2.5em",e.addEventListener("click",e=>{this.m(),XenForo.alert("Settings have been saved.",!1,5e3,console.log),setTimeout(()=>{window.location.reload()},4500)}),this.l(),this.l(),this.i.appendChild(e),document.createElement("button")),e=(e.innerHTML="Close",e.style.width="6em",e.style.height="2.5em",e.addEventListener("click",e=>{this.close()}),this.l(),this.l(),this.l(),this.l(),this.i.appendChild(e),document.body.appendChild(this.i),document.createElement("img"));e.src="data:image/png;base64,",e.id="sbe-settings-opener-img",
/* opener.style.marginLeft = '9em' */
e.style.marginTop="2px",e.style.marginLeft="2em",e.height=20,e.width=20,e.addEventListener("click",e=>{this.open()}),isOnMiddleTheme||isOnOriginalTheme?document.querySelector('[class="navTabs"]')?.insertBefore(e,document.querySelector(".visitorTabs")):document.querySelector("#topbar>.p-nav-inner>.left")?.appendChild(e)}open(){this.i?.showModal()}close(){this.i?.close()}m(){localStorage.setItem("sbe-settings",JSON.stringify({threadTitleEnabled:this.threadTitleEnabled,hideShopTab:this.hideShopTab,strikethroughBannedUsers:this.strikethroughBannedUsers,SBonlIntegration:this.SBonlIntegration,actualDateOnFrontpage:this.actualDateOnFrontpage,fixBedrockPlayersImages:this.fixBedrockPlayersImages,responsiveModals:this.responsiveModals,movePoke:this.movePoke,ratingRatio:this.ratingRatio,removeRatingCommas:this.removeRatingCommas,avatarOnProfileStats:this.avatarOnProfileStats,birthdayHatOnPFP:this.birthdayHatOnPFP,roundedFriendsOnProfile:this.roundedFriendsOnProfile,postLinkButton:this.postLinkButton,minotarNotCrafatar:this.minotarNotCrafatar,noMoreCamo:this.noMoreCamo,fadeInReactions:this.fadeInReactions,moreSearchOnCard:this.moreSearchOnCard,adBlocker:this.adBlocker,unpinLawsuit:this.unpinLawsuit,fixOldLinks:this.fixOldLinks,dontShare:this.dontShare,o:this.o}))}g(){var e=JSON.parse(localStorage.getItem("sbe-settings")??"{}");for(const t of Object.keys(e))this[t]=e[t]}}const settings=new _Settings,THEMES={u:6,p:22,NEW:30};let themes=[{name:"Dark Mode (Pink Accent)",description:"A dark mode & pink accented theme",css:/*$import*/'#content .pageContent {    background-color: #414141;}    .secondaryContent,    .avatar img,    .avatarCropper,    .recentNews,    .breadBoxTop>nav>fieldset.breadcrumb,    .breadBoxBottom>nav>fieldset.breadcrumb,    .breadcrumb .crust a.crumb,    #searchBar,    .messageUserBlock,    div.section.sectionMain,    div.messageUserBlock>.avatarHolder,    ul.tabs.mainTabs.Tabs li{    background: #2D2D2DBB !important;    color: #c5c5c5;    /* outline: 1px solid lime; */}    .messageContent,    .section.sectionMain.recentNews,    .recentNews>div.primaryContent.leftDate,    fieldset#QuickSearch,    fieldset#QuickSearch>formPopup,    .footer .pageContent,    .navTabs,    div.primaryControls,    input#QuickSearchQuery,    ul.tabs.mainTabs.Tabs,    ul.tabs.mainTabs.Tabs>li,    ul.tabs.mainTabs.Tabs>li>a,    form#ProfilePoster,    li[id^="profile-post-"],    .primaryContent,    #AccountMenu,    .mainContent>ul.tabs{    background: #323232;}.messageInfo>textarea.textCtrl:focus {    background-image: none !important;    background: #665766;}    .messageInfo>textarea.textCtrl ,    #AccountMenu>.menuHeader,    ol#forums,    ol.nodeList,    .mainContent>ul.tabs>li:not(.active)>a{    background-color: #2b2b2b;}    li.node>ol.nodeList>li.node>div.nodeInfo,    div.extraUserInfo{    background-color: #1a1a1a !important;}    .titleBar,    #serverstatus,    .visitorText,    .dark_postrating_neutral,    .sectionFooter>div.continue>a.iconKey.button,    footer>.footerLegal,    footer>.footerLegal>a,    input#QuickSearchQuery,    article>blockquote.ugc.baseHtml,    .tabs>li:not(.active)>a,    div.primaryContent.menuHeader>h3>a.concealed[href^="members/"],    ol.nodeList>li.node>div.nodeInfo,    div#pageNodeContent,    .searchResult.post>.listBlock.main>blockquote>a{    color: white !important;}    a,    .PageNavNext,    [class="PageNavNext "],    a.PreviewTooltip,    .primaryContent a,    ul.Tabs.tabs.mainTabs>li.active>a,    #AccountMenu a,    #AccountMenu .secondaryContent a,    #AccountMenu .AutoValidator label{    color: #c450c6;}    .navTabs .navTab.selected .tabLinks,    .subHeading,    .sectionFooter,    .PageNav>nav a[href^="articles/"],    li.navTab.selected,    li.node.category > div.nodeInfo,    .discord-widget > p.discord-join > a,    .mainContent>ul.tabs>li.active>a,    ul[id^="premium-"].staffTitle{    background-color: #d38fb9;}    #header,     #headerMover #headerProxy,     #content,    .sectionFooter>div.continue>a.iconKey.button,    footer    /* nav span.scrollable>span.items a[href] */{    background: #7d4f77;}    .subHeading,    li.node.category > div.nodeInfo{    border-bottom: none !important;    border-top: none !important;}.PageNav>nav a[href^="articles/"] {    color: black;}a.avatar>img[alt][src^="data/avatars/"] {    filter: brightness(0.8);}    span.arrow,    span.arrow>span{    border-left-color: thistle !important;}.discussionListItems .unread .title a {    background: url(https://static.skyblock.net/sparkles/sparkle-7.gif);}',basis:THEMES.u},{name:"Nightblock",description:"A dark theme for Skyblock Forums",css:/*$import*/'/*This file has been dedicated to the public domain by the original author nightcat (https://nightcat.gg/). Explicit permission for use in skyblock-extras has been received, regardless.All changes from the original (https://github.com/uso-archive/data/blob/flomaster/data/usercss/146532.user.css), are licensed GPL 3.0.*//* ==UserStyle==@name         Nightblock@namespace    USO Archive@author       Nightcat@description  A dark theme for Skyblock Forums.@version      20200211.16.37@license      CC0-1.0@preprocessor uso==/UserStyle== */body,    .button,    .staffTitle li,    .username .style2,    .username .style24,    .dark_postrating_neutral,    span[style="color: #000000"],    span[style="color: rgb(0, 0, 0)"],    .bbCodeQuote .attribution,    .event .content .description em,    .discussionList .sectionFooter .contentSummary {    color: #fff !important;}a {    color: #bb86fc !important;}.formPopup .controlsWrapper,    .subHeading,    .textWithCount.subHeading .text,    .discussionList .sticky .title a,    .DateTime,    .discussionList .sticky .EditControl,    .discussionList .sticky .username,    .discussionList .sticky .pairsJustified dt,    a.username.author,    .postNumber,    .discussionList .sticky .DateTime,    .UnhiddenContent > .attribution.type{    color: #03dac5 !important;}#header,    body,    .pageContent,    .mainContainer,    .navTabs,    .secondaryContent,    .breadcrumb,    .sectionMain,    .primaryContent,    .button,    .profilePage .tabs.mainTabs,    .profilePage .tabs.mainTabs li a,    .nodeInfo.categoryForumNodeInfo,    .discussionListItem .posterAvatar .avatarContainer,    .discussionListItem,    .discussionListItem .posterAvatar,    .discussionListItem .stats,    .dark_postrating_detail,    .bbCodeQuote .quoteContainer,    .bbCodeBlock pre,    .bbCodeBlock .code,    .tabs,    .tabs li a {    background: #121212 !important;}.stackAlertContent,    .navTabs .navTab.selected .tabLinks,    .navTabs .navTab.selected .navLink,    .navPopup .PopupItemLinkActive:hover,    #QuickSearch,    .primaryControls,    .secondaryControls,    .formPopup .controlsWrapper,    a.crumb,    .subHeading,    .sectionFooter,    .avatar img,    .PageNav a,    .nodeList .categoryStrip,    .staffTitle,    .profilePage .tabs.mainTabs li.active a,    .sectionHeaders,    .discussionList .sticky .posterAvatar,    .discussionList .sticky .stats,    .discussionList .sticky,    .messageUserBlock div.avatarHolder,    .messageUserBlock,    .messageUserBlock .extraUserInfo,    .bbCodeQuote .attribution,    .bbCodeBlock .type,    .redactor_toolbar,    .tabs li.active a {    background: #1f1b24 !important;}.breadcrumb .crust .arrow span {    border-left-color: #1f1b24 !important;}.subHeading,    .sectionFooter,    .nodeList .categoryStrip,    .sectionHeaders {    border: 1px solid #d7edfc !important;}.navTabs .navTab.selected .navLink {    text-shadow: 0 0 0 transparent, 0px 0px 3px #000;}.Menu {    border: 1px solid #1f1b24 !important;    border-top: 5px solid #1f1b24 !important;}.discussionList .sticky .title a,    .discussionList .sticky .DateTime,    .discussionList .sticky .EditControl,    .discussionList .sticky .username,    .discussionList .sticky .pairsJustified dt {    text-shadow: none !important;}#friends-mount>.friend {    background-color: inherit;}.UnhiddenContent blockquote {    color: black;}',basis:THEMES.u},{name:"Better New SB",description:"A better version of the new Skyblock theme",css:/*$import*/'div.navTabs {    background:#2b485c;    border-radius: 0 !important;}#landingHero>* {    display:none !important;    background: none !important;}#landingHero {    height:25px;    padding:0 !important;}#content .sidebar .section {    border-radius: 0px !important;}#content .section {    -webkit-box-shadow:none !important;    box-shadow:none !important;}.avatar img, .avatarWrap .img.s {    border-radius: 5px !important;}.visitorTabs, .navTabs .visitorTabs {    display: block !important;    }div#navigation {    border-bottom: none;}#content .sidebar .section .secondaryContent {    padding: 15px !important;}#footer>.top {    padding:25px;}#content {    background: #d1eef5 !important;}.newsText {    color: #113240}li[id^="thread"]>.title {    font-size: 12px;}.sbe-mg-top {    margin-top: 20px}a.PreviewTooltip>.prefix {    margin: 0 !important;}',basis:THEMES.p}];if(ls.getItem("customThemes")){const Q=JSON.parse(ls.getItem("customThemes")??"[]");0<Q.length&&(Q.map(e=>{null!=e.h&&(e.basis=e.h?THEMES.u:THEMES.p,delete e.h),e.v=()=>GM_addStyle(e.css??"")}),themes=[...themes,...Q])}if("true"==localStorage.getItem("customThemeMode")&&localStorage.getItem("customTheme-SBE")){const S=JSON.parse(localStorage.getItem("customTheme-SBE")??"[]");S.css?GM_addStyle(S.css??""):S.v&&S.v(),document.querySelector('.pageContent>.choosers>dd>a[href*="misc/style?redirect"]').innerHTML=S.name}if(waitForElm(".section.styleChooser").then(e=>{console.log("Style chooser opened!"),document.querySelectorAll('a[href^="misc/style"]').forEach(e=>{const o=e;o.addEventListener("click",function(e){var t=o.href.match(/style_id=(\d+)/)[1];localStorage.setItem("sbe-vanilla-style",t.toString())})}),console.log("Overlay found!");var t=e.querySelector("ol.twoColumns.primaryContent.chooserColumns");for(const a of AF(t.querySelectorAll("li"))){const i=a.querySelector("a");/style_id=(6|22|30)/.test(i?.href??"")&&i?.addEventListener("click",e=>{e.preventDefault(),localStorage.setItem("customThemeMode","false"),window.location.href=i?.href})}for(const r of themes){var o=t.querySelector("li")?.cloneNode(!0),n=o.querySelector("a"),n=(n?.removeAttribute("href"),n?.addEventListener("click",e=>{e.preventDefault(),localStorage.setItem("customThemeMode","true"),localStorage.setItem("customTheme-SBE",JSON.stringify(r)),window.location.href=`https://skyblock.net/misc/style?style_id=${r.basis}&_xfToken=${xfToken}&redirect=`+encodeURI(window.location.href)}),o.querySelector(".title")),s=o.querySelector(".description");n.innerHTML=r.name,s.innerHTML=r.description,o.classList.add("sbe-pointer"),t.appendChild(o)}}),isOnNewTheme){const ha=document.querySelector("div#footer>div.bottom>div.container");ha.innerHTML+=`<a href="misc/style?redirect${encodeURIComponent(location.pathname)}" class="changeTheme OverlayTrigger Tooltip" title="Style Chooser" rel="nofollow">Change Theme</a>`}if(console.log("skyblock extras loaded"),settings.threadTitleEnabled){const ia=document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]');ia?.textContent&&(document.title=ia.textContent+" | Skyblock Forums")}if(settings.adBlocker&&document.querySelectorAll(".adsbygoogle").forEach(e=>{e.remove()}),settings.hideShopTab){const ka=document.querySelector("ul.publicTabs"),la=ka?.children[7],ma=ka?.children[6];la&&ma?.replaceWith(la)}if(settings.strikethroughBannedUsers){const na=document.querySelectorAll(".messageUserBlock"),oa=Array.from(na).filter(e=>e.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]'));oa.forEach(e=>{e.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.SBonlIntegration&&isOnUserProfile){const ra=document.querySelector('[href="misc/quick-navigation-menu"]'),sa=ra?.cloneNode(!0),ta="http://skyblock.onl/@"+isOnUserProfile[1];sa.href=ta,sa.style.background="url('data:image/png;base64,')",ra?.parentElement?.appendChild(sa)}if(settings.actualDateOnFrontpage){
//@ts-ignore
const ua=e=>strftime("%H:%M %d/%m/%Y",e??null),va=document.querySelectorAll(".dateTime .DateTime");va.forEach(e=>{var t=e.getAttribute("data-time")??null;null!==t&&(e.innerHTML=ua(new Date(1e3*parseInt(t))))}),patchClass(XenForo.TimestampRefresh?.prototype,"refresh",(e,t,o)=>{})}if(settings.fixBedrockPlayersImages&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(e=>{e.addEventListener("click",e=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(e=>{e.getAttribute("src")?.startsWith("MHF_")||e.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.responsiveModals&&!async function(){await waitForElm(".memberCard");let n;setInterval(()=>{var e,t,o=document.body.clientWidth;between(o,o-10,o+10)&&(t=(o/2-((e=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",e.style.left=t,n=o)},500)}(),settings.movePoke&&isOnUserProfile){const Ja=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),Ka=Ja?.querySelector('li > a.OverlayTrigger[href^="pokes/"]'),La=Ka?.cloneNode(!0);// remove the dropdown if there's only the one element in the dropdown
(1==Ja?.childElementCount?Ka?.parentElement:Ka)?.remove();//otherwise only remove the poke option
let e=document.createElement("li");e.appendChild(La),document.querySelector(".followBlock > ul")?.appendChild(e),0===Ja?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}if(settings.ratingRatio&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const Na=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),Oa=Na?.querySelector("tbody");Na.style.padding="5px 20px",Array.from(Oa?.children??[]).forEach(e=>{var t,o,n;e.querySelector("th")?((t=document.createElement("th")).innerHTML="Ratio:",e.appendChild(t)):(t=e.children[1],o=e.children[2],(n=document.createElement("td")).setAttribute("class",o.getAttribute("class")??"dark_postrating_neutral"),n.innerHTML=calculateRatio(parseInt(t.innerHTML.replace(/,/g,"")),parseInt(o.innerHTML.replace(/,/g,""))),e.appendChild(n))})}if(settings.removeRatingCommas&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(e=>{e.innerHTML.replace(/,/g,"")}),settings.avatarOnProfileStats&&isOnUserProfile){let e=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');e.style.position="relative",e.children[0].style.position="absolute"}if(settings.birthdayHatOnPFP){
// https://www.flaticon.com/free-icon/party-hat_2668968
const Wa="data:image/png;base64,",Xa=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(e=>{var t,o,n;"Birthday:"===e.children[0].innerHTML&&(t=(e=colonNumber(e.children[1].innerHTML.split(" "),2))[0],e=parseInt(e[1]??1),t=getMonthFromString(t),[o,n]=[(o=new Date).getMonth()+1,o.getDate()],e===n)&&t===o&&(Xa.style.position="relative",(e=document.createElement("img")).src=Wa,e.width=256,e.height=256,e.style.width="256px",e.style.height="256px",e.style.position="absolute",e.style.zIndex="999",e.style.left="100px",e.style.top="-420px",e.style.rotate="30deg",e.style.pointerEvents="none",Xa.appendChild(e))})}settings.roundedFriendsOnProfile&&isOnUserProfile&&GM_addStyle(".friend>.friend-head>img { border-radius: 7px; };"),settings.postLinkButton&&isOnThread&&(AF(document.querySelectorAll('li[id^="post-"].message[data-author]')).forEach(e=>{var t=e.id.split("-")[1],e=e.querySelector(".publicControls");const o=document.createElement("a");o.href="https://skyblock.net/posts/"+t,o.setAttribute("class","ReplyQuote item control reply"),o.title="Copy link to this message.",o.innerHTML="<span></span>Copy Link",o.onclick=()=>window.navigator.clipboard.writeText(o.href),e?.appendChild(o)}),
// Fix copy link showing up in the selected text hover thing
patchClass(XenForo.SelectQuotable?.prototype,"createButton",function(e,t){var e=e(),o=this.$button[0];return 3==o.children.length&&// 3 includes copy link
o.removeChild(o.lastElementChild),e})),settings.postLinkButton&&isOnUserProfile&&AF(document.querySelectorAll('li[id^="profile-post-"].messageSimple[data-author]>.messageInfo>.messageMeta>.publicControls')).forEach(e=>{
/* self-comments won't have like, but for ones with like we still want to insert before like */
var e=e.querySelector("a.LikeLink.like")??e.querySelector("a.CommentPoster.postComment"),t=e.href.replace("https://skyblock.net/","").split("/")[1];console.log(e,t);const o=document.createElement("a");o.classList.add("item","control","copylink"),o.href="profile-posts/"+t,o.onclick=()=>navigator.clipboard.writeText(o.href);t=document.createElement("span");t.innerHTML="Copy Link",o.appendChild(t),e.insertAdjacentElement("beforebegin",o)}),settings.minotarNotCrafatar&&setTimeout(()=>{AF(document.querySelectorAll('img[src^="https://crafatar.com/avatars"]')).forEach(e=>{console.log(e),e.setAttribute("src",(e.getAttribute("src")??"").replace(/https\:\/\/crafatar.com\/avatars\/([a-fA-F0-9\-]{32,36})\?size=(\d+)&overlay=true/g,"https://minotar.net/avatar/$1/$2"))})},1e3),settings.noMoreCamo&&setTimeout(()=>{AF(document.querySelectorAll('[src*="camo.skyblock.net/"]')).forEach(e=>{var t=decodeURIComponent(e.getAttribute("src")??"").split("?url"),t=decodeURIComponent(2==t.length?t[1]:t[2]),t=t.startsWith("=")?t.slice(1):t;e.setAttribute("src",t)})},1e3),settings.fadeInReactions&&GM_addStyle(".dark_postrating_inputlist {transition: opacity 0.5s;}.dark_postrating_inputlist:not(:hover) {opacity: 0.1 !important;}"),settings.moreSearchOnCard&&patchClass(XenForo.OverlayLoader?.prototype,"createOverlay",(e,t)=>{var o;return t.templateHtml?t.templateHtml.includes('<div id="memberCard')&&(o=`<dt>Threads: </dt><dd><a href="search/member?user_id=${t.templateHtml.match(/<a href="members\/.+\.([0-9]+)\/"\>Profile Page<\/a>/)?.[1]??"1"}&content=thread" class="concealed" rel="nofollow">Search</a></dd>`,t.templateHtml=insert(t.templateHtml,t.templateHtml.indexOf("\x3c!-- slot: pre_likes"),o)):console.log("createoverlay does not contain templatehtml: "+t),e(t)}),settings.unpinLawsuit&&isOnIndex&&document.querySelector('[id="recentNews"]>[id="145369"]')?.remove(),settings.fixOldLinks&&document.querySelectorAll('a[href*="block.net"]').forEach(e=>{console.log;e.href=e.href
/* replace a http(s) (or no protocol) url with, or without www. on endblock.net, hellblock.net or skyblock.net to be https://skyblock.net */.replace(/^((?:https?:\/\/)?(?:w{3})?\.(?:end|hell|sky)block\.net)/g,"https://skyblock.net").replaceAll("skyblock.net/index.php?threads","skyblock.net/threads")}),settings.dontShare&&GM_addStyle("div.sharePage {display: none;}"),settings.o&&isOnThread&&[...AF(document.querySelectorAll('li[id^="post-"].message[data-author]')),...AF(document.querySelectorAll('li[id^="message-"].message[data-author]'))].forEach(e=>{const o=e.id.split("-")[1];var e=e.querySelector(".publicControls"),t=document.createElement("a");t.href="#",t.setAttribute("class","ReplyQuote item control reply"),t.title="Copy message BBCode.",t.innerHTML="<span></span>Copy BBCode",t.onclick=async e=>{e.preventDefault();var e=await(await fetch(`https://skyblock.net/posts/${o}/quote`,{headers:{k:"application/x-www-form-urlencoded"},body:"_xfResponseType=json&_xfToken="+xfToken,method:"POST"})).json(),t=e.quote?e.quote.trim().match(/^\[QUOTE="[^,]+, post: \d+, member: \d+"\](.+)\[\/QUOTE]$/s):"";if(!e.quote||!t)return console.log("body.quote",e.quote),console.log("quoteunquoted",t),window.navigator.clipboard.writeText("<failed to get content>");window.navigator.clipboard.writeText(t[1])},e?.appendChild(t)});
