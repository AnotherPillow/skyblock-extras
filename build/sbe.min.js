// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @description A userscript to improve the skyblock.net forums experience!
// @match       https://skyblock.net/*
// @grant       none
// @version     1.0
// @author      AnotherPillow
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// ==/UserScript==
"use strict";function waitForElm(n){return new Promise(t=>{if(document.querySelector(n))return t(document.querySelector(n));const e=new MutationObserver(A=>{document.querySelector(n)&&(e.disconnect(),t(document.querySelector(n)))});e.observe(document.body,{childList:!0,subtree:!0})})}function between(A,t,e){return t<=A&&A<=e}const isOnUserProfile=window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1;
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(A,t){for(var e=t;1<e;e--)A%e==0&&t%e==0&&(A/=e,t/=e);return A+":"+t}const AF=Array.from,colonNumber=(A,e)=>A.filter((A,t)=>t<e).filter(A=>""!==A),getMonthFromString=A=>new Date(Date.parse(A+" 1, 2012")).getMonth()+1;class _Settings{A=!0;t=!0;o=!0;i=!0;g=!0;v=!0;m=!0;l=!0;B=!0;I=!0;C=!0;P=!1;u=!0;p;h(A,t){var e="sbe-setting-"+t.toString().replace(/\s/g,"_"),n=document.createElement("label"),A=(n.innerHTML=A+": ",n.setAttribute("for",e),document.createElement("input"));A.name=e,A.id=e,A.type="checkbox",
//@ts-ignore
A.checked=this[t]??!1,A.addEventListener("click",A=>{A=//@ts-ignore
A.target.checked;this[t]=A}),this.p?.appendChild(n),this.p?.appendChild(A),this.k()}k(){this.p?.appendChild(document.createElement("br"))}constructor(){this.H(),this.p=document.createElement("dialog");var A=document.createElement("h1"),A=(A.innerHTML="Skyblock Extras Settings",A.style.fontSize="2em",this.p.appendChild(A),this.k(),this.p.style.width="640px",this.p.style.height="480px",this.p.style.borderRadius="1em",this.p.style.border="none",this.p.style.outline="none",this.p.style.textAlign="center",this.h("Thread Title as Browser Title","threadTitleEnabled"),this.h("Remove the shop tab","hideShopTab"),this.h("Strike through banned users' names","strikethroughBannedUsers"),this.h("Better New Style Theme","betterNewSB"),this.h("Skyblock.onl Integration","SBonlIntegration"),this.h("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.h("Fix bedrock players' images","fixBedrockPlayersImages"),this.h("Responsive Modals","responsiveModals"),this.h("Move poking out of moderator tools","movePoke"),this.h("Add given:received ratio for reactions on profile","ratingRatio"),this.h("Remove commas from ratings","removeRatingCommas"),this.h("Avatar on profile stats.","avatarOnProfileStats"),this.h("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),document.createElement("button")),A=(A.innerHTML="Save",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.S(),window.location.reload()}),this.k(),this.k(),this.p.appendChild(A),document.createElement("button")),A=(A.innerHTML="Close",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.close()}),this.k(),this.k(),this.k(),this.k(),this.p.appendChild(A),document.body.appendChild(this.p),document.createElement("img"));A.src=" data:image/png;base64,",A.id="sbe-settings-opener-img",A.style.marginLeft="9em",A.style.marginTop="2px",A.height=20,A.width=20,A.addEventListener("click",A=>{this.open()}),document.querySelector('[class="navTabs"]')?.insertBefore(A,document.querySelector(".visitorTabs"))}open(){this.p?.showModal()}close(){this.p?.close()}S(){localStorage.setItem("sbe-settings",JSON.stringify({A:this.A,t:this.t,o:this.o,i:this.i,g:this.g,v:this.v,m:this.m,l:this.l,B:this.B,I:this.I,C:this.C,P:this.P,u:this.u}))}H(){var A=JSON.parse(localStorage.getItem("sbe-settings")??"{}");for(const t of Object.keys(A))
//@ts-ignore
this[t]=A[t]}}const settings=new _Settings;if(document.querySelector(".navTabs")){const I=`
        .navTabs {
            position: relative;
        }

        .navTabs::before {
            content: "+";
            position: absolute;
            z-index: 20;
            color: white;
            top: -0.7rem;
            left: 5px;
            font-size: 3em;
        }
    `;GM_addStyle(I)}if(settings.A&&(document.title=(document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]'))?.textContent+" | Skyblock Forums"),settings.t){const J=document.querySelector("ul.publicTabs"),K=J?.children[7],L=J?.children[6];K&&L?.replaceWith(K)}if(settings.o){const M=document.querySelectorAll(".messageUserBlock"),N=Array.from(M).filter(A=>A.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]')),O=`
        .sbe-strikethrough {
            text-decoration: line-through !important;
            text-decoration-thickness: 2px !important;
        }
    `;GM_addStyle(O),N.forEach(A=>{A.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.i){const R=`
    div.navTabs {
        background:#2b485c;
        border-radius: 0 !important;
    }
    #landingHero>* {
        display:none !important;
        background: none !important;
    }
    #landingHero {
        height:25px;
        padding:0 !important;
    }
    #content .sidebar .section {
        border-radius: 0px !important;
    }
    #content .section {
        -webkit-box-shadow:none !important;
        box-shadow:none !important;
    }
    .avatar img, .avatarWrap .img.s {
        border-radius: 5px !important;
    }
    .visitorTabs, .navTabs .visitorTabs {
        display: block !important;    
    }
    div#navigation {
        border-bottom: none;
    }
    #content .sidebar .section .secondaryContent {
        padding: 15px !important;
    }
    #footer>.top {
        padding:25px;
    }
    #content {
        background: #d1eef5 !important;
    }
    .newsText {
        color: #113240
    }
    li[id^="thread"]>.title {
        font-size: 12px;
    }
    .sbe-mg-top {
        margin-top: 20px
    }

    a.PreviewTooltip>.prefix {
        margin: 0 !important;
    }
    `;document.querySelector('[data-clipboard-text="play.skyblock.net"]')&&GM_addStyle(R)}if(settings.g&&isOnUserProfile){const S=document.querySelector('[href="misc/quick-navigation-menu"]'),T=S?.cloneNode(!0),U="http://skyblock.onl/@"+isOnUserProfile[1];T.href=U,T.style.background="url('data:image/png;base64,')",S?.parentElement?.appendChild(T)}if(settings.v){
//@ts-ignore
const V=A=>strftime("%H:%M %d/%m/%Y",A??null),W=document.querySelectorAll(".dateTime .DateTime");W.forEach(A=>{var t=A.getAttribute("data-time")??null;null!==t&&(A.innerHTML=V(new Date(1e3*parseInt(t))))})}if(settings.m&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(A=>{A.addEventListener("click",A=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(A=>{A.getAttribute("src")?.startsWith("MHF_")||A.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.l&&!async function(){await waitForElm(".memberCard");let n;setInterval(()=>{var A,t,e=document.body.clientWidth;between(e,e-10,e+10)&&(t=(e/2-((A=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",console.log(A),A.style.left=t,n=e)},500)}(),settings.B&&isOnUserProfile){const fa=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),ga=fa?.querySelector('li > a.OverlayTrigger[href^="pokes/"]'),ha=ga?.cloneNode(!0);ga?.parentElement?.remove();let A=document.createElement("li");A.appendChild(ha),document.querySelector(".followBlock > ul")?.appendChild(A),0===fa?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}if(settings.I&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const ja=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),ka=ja?.querySelector("tbody");ja.style.padding="5px 20px",console.log(ka),Array.from(ka?.children??[]).forEach(A=>{var t,e,n;console.log(A),A.querySelector("th")?((t=document.createElement("th")).innerHTML="Ratio:",A.appendChild(t)):(t=A.children[1],e=A.children[2],console.log(t,e),(n=document.createElement("td")).setAttribute("class",e.getAttribute("class")??"dark_postrating_neutral"),n.innerHTML=calculateRatio(parseInt(t.innerHTML.replace(/,/g,"")),parseInt(e.innerHTML.replace(/,/g,""))),A.appendChild(n))})}if(settings.C&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(A=>{A.innerHTML.replace(/,/g,"")}),settings.P&&isOnUserProfile){let A=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');A.style.position="relative",A.children[0].style.position="absolute"}if(settings.u){
// https://www.flaticon.com/free-icon/party-hat_2668968
const sa="data:image/png;base64,",ta=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(A=>{var t,e,n;"Birthday:"===A.children[0].innerHTML&&(t=(A=colonNumber(A.children[1].innerHTML.split(" "),2))[0],A=parseInt(A[1]??1),t=getMonthFromString(t),[e,n]=[(e=new Date).getMonth()+1,e.getDate()],A===n)&&t===e&&(ta.style.position="relative",(A=document.createElement("img")).src=sa,A.width=256,A.height=256,A.style.width="256px",A.style.height="256px",A.style.position="absolute",A.style.zIndex="999",A.style.left="100px",A.style.top="-420px",A.style.rotate="30deg",A.style.pointerEvents="none",ta.appendChild(A))})}
