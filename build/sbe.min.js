// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @description A userscript to improve the skyblock.net forums experience!
// @match       https://skyblock.net/*
// @grant       none
// @version     1.0
// @author      AnotherPillow
// @license     GNU GPLv3
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// ==/UserScript==
"use strict";function waitForElm(n){return new Promise(t=>{if(document.querySelector(n))return t(document.querySelector(n));const e=new MutationObserver(A=>{document.querySelector(n)&&(e.disconnect(),t(document.querySelector(n)))});e.observe(document.body,{childList:!0,subtree:!0})})}function between(A,t,e){return t<=A&&A<=e}
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(A,t){for(var e=t;1<e;e--)A%e==0&&t%e==0&&(A/=e,t/=e);return A+":"+t}const AF=Array.from,colonNumber=(A,e)=>A.filter((A,t)=>t<e).filter(A=>""!==A),getMonthFromString=A=>new Date(Date.parse(A+" 1, 2012")).getMonth()+1,getHrefWithoutAnchor=()=>window.location.href.replace(new RegExp(window.location.hash+"$"),""),isOnThread=getHrefWithoutAnchor().match(/https\:\/\/skyblock\.net\/threads\/.+\.\d+\/?/),isOnUserProfile=window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1;class _Settings{threadTitleEnabled=!0;hideShopTab=!0;strikethroughBannedUsers=!0;betterNewSB=!0;SBonlIntegration=!0;actualDateOnFrontpage=!0;fixBedrockPlayersImages=!0;responsiveModals=!0;movePoke=!0;ratingRatio=!0;removeRatingCommas=!0;avatarOnProfileStats=!1;birthdayHatOnPFP=!0;roundedFriendsOnProfile=!0;postLinkButton=!0;A;t(A,t){var e="sbe-setting-"+t.toString().replace(/\s/g,"_"),n=document.createElement("label"),A=(n.innerHTML=A+": ",n.setAttribute("for",e),document.createElement("input"));A.name=e,A.id=e,A.type="checkbox",
//@ts-ignore
A.checked=this[t]??!1,A.addEventListener("click",A=>{A=//@ts-ignore
A.target.checked;this[t]=A}),this.A?.appendChild(n),this.A?.appendChild(A),this.o()}o(){this.A?.appendChild(document.createElement("br"))}constructor(){this.i(),this.A=document.createElement("dialog");var A=document.createElement("h1"),A=(A.innerHTML="Skyblock Extras Settings",A.style.fontSize="2em",this.A.appendChild(A),this.o(),this.A.style.width="640px",this.A.style.height="480px",this.A.style.borderRadius="1em",this.A.style.border="none",this.A.style.outline="none",this.A.style.textAlign="center",this.t("Thread Title as Browser Title","threadTitleEnabled"),this.t("Remove the shop tab","hideShopTab"),this.t("Strike through banned users' names","strikethroughBannedUsers"),this.t("Better New Style Theme","betterNewSB"),this.t("Skyblock.onl Integration","SBonlIntegration"),this.t("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.t("Fix bedrock players' images","fixBedrockPlayersImages"),this.t("Responsive Modals","responsiveModals"),this.t("Move poking out of moderator tools","movePoke"),this.t("Add given:received ratio for reactions on profile","ratingRatio"),this.t("Remove commas from ratings","removeRatingCommas"),this.t("Avatar on profile stats.","avatarOnProfileStats"),this.t("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),this.t("Round friends' names on profile.","roundedFriendsOnProfile"),this.t("Add button to copy link to post on posts.","postLinkButton"),document.createElement("button")),A=(A.innerHTML="Save",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.g(),window.location.reload()}),this.o(),this.o(),this.A.appendChild(A),document.createElement("button")),A=(A.innerHTML="Close",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.close()}),this.o(),this.o(),this.o(),this.o(),this.A.appendChild(A),document.body.appendChild(this.A),document.createElement("img"));A.src=" data:image/png;base64,",A.id="sbe-settings-opener-img",A.style.marginLeft="9em",A.style.marginTop="2px",A.height=20,A.width=20,A.addEventListener("click",A=>{this.open()}),document.querySelector('[class="navTabs"]')?.insertBefore(A,document.querySelector(".visitorTabs"))}open(){this.A?.showModal()}close(){this.A?.close()}g(){localStorage.setItem("sbe-settings",JSON.stringify({threadTitleEnabled:this.threadTitleEnabled,hideShopTab:this.hideShopTab,strikethroughBannedUsers:this.strikethroughBannedUsers,betterNewSB:this.betterNewSB,SBonlIntegration:this.SBonlIntegration,actualDateOnFrontpage:this.actualDateOnFrontpage,fixBedrockPlayersImages:this.fixBedrockPlayersImages,responsiveModals:this.responsiveModals,movePoke:this.movePoke,ratingRatio:this.ratingRatio,removeRatingCommas:this.removeRatingCommas,avatarOnProfileStats:this.avatarOnProfileStats,birthdayHatOnPFP:this.birthdayHatOnPFP,roundedFriendsOnProfile:this.roundedFriendsOnProfile,postLinkButton:this.postLinkButton}));
// alert(localStorage)
// debugger
}i(){var A=JSON.parse(localStorage.getItem("sbe-settings")??"{}");
// alert(localStorage.getItem('sbe-settings'))
for(const t of Object.keys(A))this[t]=A[t];
// debugger
}}const settings=new _Settings;if(document.querySelector(".navTabs")){const H=`
        .navTabs {
            position: relative;
        }

        .navTabs::before {
            content: "+";
            position: absolute;
            z-index: 20;
            color: white;
            top: -0.7rem;
            left: 5px;
            font-size: 3em;
        }
    `;GM_addStyle(H)}if(settings.threadTitleEnabled&&(document.title=(document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]'))?.textContent+" | Skyblock Forums"),settings.hideShopTab){const I=document.querySelector("ul.publicTabs"),J=I?.children[7],K=I?.children[6];J&&K?.replaceWith(J)}if(settings.strikethroughBannedUsers){const L=document.querySelectorAll(".messageUserBlock"),M=Array.from(L).filter(A=>A.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]')),N=`
        .sbe-strikethrough {
            text-decoration: line-through !important;
            text-decoration-thickness: 2px !important;
        }
    `;GM_addStyle(N),M.forEach(A=>{A.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.betterNewSB){const Q=`
    div.navTabs {
        background:#2b485c;
        border-radius: 0 !important;
    }
    #landingHero>* {
        display:none !important;
        background: none !important;
    }
    #landingHero {
        height:25px;
        padding:0 !important;
    }
    #content .sidebar .section {
        border-radius: 0px !important;
    }
    #content .section {
        -webkit-box-shadow:none !important;
        box-shadow:none !important;
    }
    .avatar img, .avatarWrap .img.s {
        border-radius: 5px !important;
    }
    .visitorTabs, .navTabs .visitorTabs {
        display: block !important;    
    }
    div#navigation {
        border-bottom: none;
    }
    #content .sidebar .section .secondaryContent {
        padding: 15px !important;
    }
    #footer>.top {
        padding:25px;
    }
    #content {
        background: #d1eef5 !important;
    }
    .newsText {
        color: #113240
    }
    li[id^="thread"]>.title {
        font-size: 12px;
    }
    .sbe-mg-top {
        margin-top: 20px
    }

    a.PreviewTooltip>.prefix {
        margin: 0 !important;
    }
    `;document.querySelector('[data-clipboard-text="play.skyblock.net"]')&&GM_addStyle(Q)}if(settings.SBonlIntegration&&isOnUserProfile){const R=document.querySelector('[href="misc/quick-navigation-menu"]'),S=R?.cloneNode(!0),T="http://skyblock.onl/@"+isOnUserProfile[1];S.href=T,S.style.background="url('data:image/png;base64,')",R?.parentElement?.appendChild(S)}if(settings.actualDateOnFrontpage){
//@ts-ignore
const U=A=>strftime("%H:%M %d/%m/%Y",A??null),V=document.querySelectorAll(".dateTime .DateTime");V.forEach(A=>{var t=A.getAttribute("data-time")??null;null!==t&&(A.innerHTML=U(new Date(1e3*parseInt(t))))})}if(settings.fixBedrockPlayersImages&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(A=>{A.addEventListener("click",A=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(A=>{A.getAttribute("src")?.startsWith("MHF_")||A.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.responsiveModals&&!async function(){await waitForElm(".memberCard");let n;setInterval(()=>{var A,t,e=document.body.clientWidth;between(e,e-10,e+10)&&(t=(e/2-((A=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",console.log(A),A.style.left=t,n=e)},500)}(),settings.movePoke&&isOnUserProfile){const ea=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),fa=ea?.querySelector('li > a.OverlayTrigger[href^="pokes/"]'),ga=fa?.cloneNode(!0);fa?.parentElement?.remove();let A=document.createElement("li");A.appendChild(ga),document.querySelector(".followBlock > ul")?.appendChild(A),0===ea?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}if(settings.ratingRatio&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const ia=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),ja=ia?.querySelector("tbody");ia.style.padding="5px 20px",console.log(ja),Array.from(ja?.children??[]).forEach(A=>{var t,e,n;console.log(A),A.querySelector("th")?((t=document.createElement("th")).innerHTML="Ratio:",A.appendChild(t)):(t=A.children[1],e=A.children[2],console.log(t,e),(n=document.createElement("td")).setAttribute("class",e.getAttribute("class")??"dark_postrating_neutral"),n.innerHTML=calculateRatio(parseInt(t.innerHTML.replace(/,/g,"")),parseInt(e.innerHTML.replace(/,/g,""))),A.appendChild(n))})}if(settings.removeRatingCommas&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(A=>{A.innerHTML.replace(/,/g,"")}),settings.avatarOnProfileStats&&isOnUserProfile){let A=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');A.style.position="relative",A.children[0].style.position="absolute"}if(settings.birthdayHatOnPFP){
// https://www.flaticon.com/free-icon/party-hat_2668968
const ra="data:image/png;base64,",sa=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(A=>{var t,e,n;"Birthday:"===A.children[0].innerHTML&&(t=(A=colonNumber(A.children[1].innerHTML.split(" "),2))[0],A=parseInt(A[1]??1),t=getMonthFromString(t),[e,n]=[(e=new Date).getMonth()+1,e.getDate()],A===n)&&t===e&&(sa.style.position="relative",(A=document.createElement("img")).src=ra,A.width=256,A.height=256,A.style.width="256px",A.style.height="256px",A.style.position="absolute",A.style.zIndex="999",A.style.left="100px",A.style.top="-420px",A.style.rotate="30deg",A.style.pointerEvents="none",sa.appendChild(A))})}settings.roundedFriendsOnProfile&&isOnUserProfile&&GM_addStyle(`
        .friend>.friend-head>img { border-radius: 7px; };
    `),settings.postLinkButton&&isOnThread&&AF(document.querySelectorAll('li[id^="post-"].message[data-author]')).forEach(A=>{var t=A["id"],A=A.querySelector(".publicControls");const e=document.createElement("a");e.href=isOnThread+"#"+t,e.setAttribute("class","ReplyQuote item control reply"),e.title="Copy link to this message.",e.innerHTML="<span></span>Copy Link",e.onclick=()=>window.navigator.clipboard.writeText(e.href),A?.appendChild(e)});
