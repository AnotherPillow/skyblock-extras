// ==UserScript==
// @name        Skyblock Extras
// @namespace   anotherpillow
// @description A userscript to improve the skyblock.net forums experience!
// @match       https://skyblock.net/*
// @grant       none
// @version     1.0
// @author      AnotherPillow
// @license     GNU GPLv3
// @require     https://cdn.jsdelivr.net/npm/@violentmonkey/dom@2
// @require     https://raw.githubusercontent.com/thdoan/strftime/master/strftime.js
// @grant       GM_addStyle
// ==/UserScript==
"use strict";function waitForElm(n){return new Promise(e=>{if(document.querySelector(n))return e(document.querySelector(n));const t=new MutationObserver(A=>{document.querySelector(n)&&(t.disconnect(),e(document.querySelector(n)))});t.observe(document.body,{childList:!0,subtree:!0})})}function between(A,e,t){return e<=A&&A<=t}const isOnUserProfile=window.location.href.match(/https\:\/\/skyblock\.net\/members\/([a-zA-Z0-9_\.]+)\.\d+/)??!1;
// https://www.codespeedy.com/get-the-ratio-of-two-numbers-in-javascript/
function calculateRatio(A,e){for(var t=e;1<t;t--)A%t==0&&e%t==0&&(A/=t,e/=t);return A+":"+e}const AF=Array.from,colonNumber=(A,t)=>A.filter((A,e)=>e<t).filter(A=>""!==A),getMonthFromString=A=>new Date(Date.parse(A+" 1, 2012")).getMonth()+1;class _Settings{A=!0;t=!0;o=!0;i=!0;g=!0;v=!0;l=!0;m=!0;B=!0;P=!0;I=!0;C=!1;u=!0;h=!0;p;k(A,e){var t="sbe-setting-"+e.toString().replace(/\s/g,"_"),n=document.createElement("label"),A=(n.innerHTML=A+": ",n.setAttribute("for",t),document.createElement("input"));A.name=t,A.id=t,A.type="checkbox",
//@ts-ignore
A.checked=this[e]??!1,A.addEventListener("click",A=>{A=//@ts-ignore
A.target.checked;this[e]=A}),this.p?.appendChild(n),this.p?.appendChild(A),this.H()}H(){this.p?.appendChild(document.createElement("br"))}constructor(){this.S(),this.p=document.createElement("dialog");var A=document.createElement("h1"),A=(A.innerHTML="Skyblock Extras Settings",A.style.fontSize="2em",this.p.appendChild(A),this.H(),this.p.style.width="640px",this.p.style.height="480px",this.p.style.borderRadius="1em",this.p.style.border="none",this.p.style.outline="none",this.p.style.textAlign="center",this.k("Thread Title as Browser Title","threadTitleEnabled"),this.k("Remove the shop tab","hideShopTab"),this.k("Strike through banned users' names","strikethroughBannedUsers"),this.k("Better New Style Theme","betterNewSB"),this.k("Skyblock.onl Integration","SBonlIntegration"),this.k("Show actual date on threads on the frontpage","actualDateOnFrontpage"),this.k("Fix bedrock players' images","fixBedrockPlayersImages"),this.k("Responsive Modals","responsiveModals"),this.k("Move poking out of moderator tools","movePoke"),this.k("Add given:received ratio for reactions on profile","ratingRatio"),this.k("Remove commas from ratings","removeRatingCommas"),this.k("Avatar on profile stats.","avatarOnProfileStats"),this.k("Place birthday hats on birthday peoples' PFPs","birthdayHatOnPFP"),this.k("Round friends' names on profile.","roundedFriendsOnProfile"),document.createElement("button")),A=(A.innerHTML="Save",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.J(),window.location.reload()}),this.H(),this.H(),this.p.appendChild(A),document.createElement("button")),A=(A.innerHTML="Close",A.style.width="6em",A.style.height="2.5em",A.addEventListener("click",A=>{this.close()}),this.H(),this.H(),this.H(),this.H(),this.p.appendChild(A),document.body.appendChild(this.p),document.createElement("img"));A.src=" data:image/png;base64,",A.id="sbe-settings-opener-img",A.style.marginLeft="9em",A.style.marginTop="2px",A.height=20,A.width=20,A.addEventListener("click",A=>{this.open()}),document.querySelector('[class="navTabs"]')?.insertBefore(A,document.querySelector(".visitorTabs"))}open(){this.p?.showModal()}close(){this.p?.close()}J(){localStorage.setItem("sbe-settings",JSON.stringify({A:this.A,t:this.t,o:this.o,i:this.i,g:this.g,v:this.v,l:this.l,m:this.m,B:this.B,P:this.P,I:this.I,C:this.C,u:this.u,h:this.h}));
// alert(localStorage)
// debugger
}S(){var A=JSON.parse(localStorage.getItem("sbe-settings")??"{}");
// alert(localStorage.getItem('sbe-settings'))
for(const e of Object.keys(A))switch(e){case"threadTitleEnabled":this.A=A[e];break;case"hideShopTab":this.t=A[e];break;case"strikethroughBannedUsers":this.o=A[e];break;case"betterNewSB":this.i=A[e];break;case"SBonlIntegration":this.g=A[e];break;case"actualDateOnFrontpage":this.v=A[e];break;case"fixBedrockPlayersImages":this.l=A[e];break;case"responsiveModals":this.m=A[e];break;case"threadTitleEnabled":this.A=A[e];break;case"responsiveModals":this.m=A[e];break;case"movePoke":this.B=A[e];break;case"ratingRatio":this.P=A[e];break;case"removeRatingCommas":this.I=A[e];break;case"avatarOnProfileStats":this.C=A[e];break;case"birthdayHatOnPFP":this.u=A[e];break;case"roundedFriendsOnProfile":this.h=A[e]}
// debugger
}}const settings=new _Settings;if(document.querySelector(".navTabs")){const H=`
        .navTabs {
            position: relative;
        }

        .navTabs::before {
            content: "+";
            position: absolute;
            z-index: 20;
            color: white;
            top: -0.7rem;
            left: 5px;
            font-size: 3em;
        }
    `;GM_addStyle(H)}if(settings.A&&(document.title=(document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]'))?.textContent+" | Skyblock Forums"),settings.t){const I=document.querySelector("ul.publicTabs"),J=I?.children[7],K=I?.children[6];J&&K?.replaceWith(J)}if(settings.o){const L=document.querySelectorAll(".messageUserBlock"),M=Array.from(L).filter(A=>A.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]')),N=`
        .sbe-strikethrough {
            text-decoration: line-through !important;
            text-decoration-thickness: 2px !important;
        }
    `;GM_addStyle(N),M.forEach(A=>{A.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.i){const Q=`
    div.navTabs {
        background:#2b485c;
        border-radius: 0 !important;
    }
    #landingHero>* {
        display:none !important;
        background: none !important;
    }
    #landingHero {
        height:25px;
        padding:0 !important;
    }
    #content .sidebar .section {
        border-radius: 0px !important;
    }
    #content .section {
        -webkit-box-shadow:none !important;
        box-shadow:none !important;
    }
    .avatar img, .avatarWrap .img.s {
        border-radius: 5px !important;
    }
    .visitorTabs, .navTabs .visitorTabs {
        display: block !important;    
    }
    div#navigation {
        border-bottom: none;
    }
    #content .sidebar .section .secondaryContent {
        padding: 15px !important;
    }
    #footer>.top {
        padding:25px;
    }
    #content {
        background: #d1eef5 !important;
    }
    .newsText {
        color: #113240
    }
    li[id^="thread"]>.title {
        font-size: 12px;
    }
    .sbe-mg-top {
        margin-top: 20px
    }

    a.PreviewTooltip>.prefix {
        margin: 0 !important;
    }
    `;document.querySelector('[data-clipboard-text="play.skyblock.net"]')&&GM_addStyle(Q)}if(settings.g&&isOnUserProfile){const R=document.querySelector('[href="misc/quick-navigation-menu"]'),S=R?.cloneNode(!0),T="http://skyblock.onl/@"+isOnUserProfile[1];S.href=T,S.style.background="url('data:image/png;base64,')",R?.parentElement?.appendChild(S)}if(settings.v){
//@ts-ignore
const U=A=>strftime("%H:%M %d/%m/%Y",A??null),V=document.querySelectorAll(".dateTime .DateTime");V.forEach(A=>{var e=A.getAttribute("data-time")??null;null!==e&&(A.innerHTML=U(new Date(1e3*parseInt(e))))})}if(settings.l&&document.querySelectorAll('a[class="PlayerListLink"][href="javascript:void(0)"]').forEach(A=>{A.addEventListener("click",A=>{document.querySelectorAll('a.avatar > img[alt^="."]').forEach(A=>{A.getAttribute("src")?.startsWith("MHF_")||A.setAttribute("src","https://minotar.net/avatar/MHF_Steve/18.png")})})}),settings.m&&!async function(){await waitForElm(".memberCard");let n;setInterval(()=>{var A,e,t=document.body.clientWidth;between(t,t-10,t+10)&&(e=(t/2-((A=document.querySelector(".memberCard"))?.clientWidth??1)/2).toString()+"px",console.log(A),A.style.left=e,n=t)},500)}(),settings.B&&isOnUserProfile){const ea=document.querySelector('div[id^="XenForoUniq"].Menu > ul.secondaryContent.blockLinksList'),fa=ea?.querySelector('li > a.OverlayTrigger[href^="pokes/"]'),ga=fa?.cloneNode(!0);fa?.parentElement?.remove();let A=document.createElement("li");A.appendChild(ga),document.querySelector(".followBlock > ul")?.appendChild(A),0===ea?.children.length&&document.querySelector(".Popup.moderatorToolsPopup")?.remove()}if(settings.P&&isOnUserProfile){document.querySelector('div[class="mast"]').style.width="230px",document.querySelector('div[class="mainProfileColumn"]').style.marginLeft="240px";const ia=document.querySelector('div.section > div.primaryContent[style="padding:0"] > table.dark_postrating_member'),ja=ia?.querySelector("tbody");ia.style.padding="5px 20px",console.log(ja),Array.from(ja?.children??[]).forEach(A=>{var e,t,n;console.log(A),A.querySelector("th")?((e=document.createElement("th")).innerHTML="Ratio:",A.appendChild(e)):(e=A.children[1],t=A.children[2],console.log(e,t),(n=document.createElement("td")).setAttribute("class",t.getAttribute("class")??"dark_postrating_neutral"),n.innerHTML=calculateRatio(parseInt(e.innerHTML.replace(/,/g,"")),parseInt(t.innerHTML.replace(/,/g,""))),A.appendChild(n))})}if(settings.I&&[...AF(document.querySelectorAll(".dark_postrating_positive")),...AF(document.querySelectorAll(".dark_postrating_neutral")),...AF(document.querySelectorAll(".dark_postrating_negative"))].forEach(A=>{A.innerHTML.replace(/,/g,"")}),settings.C&&isOnUserProfile){let A=document.querySelector('a[class^="Av"].OverlayTrigger[href="account/avatar"]');A.style.position="relative",A.children[0].style.position="absolute"}if(settings.u){
// https://www.flaticon.com/free-icon/party-hat_2668968
const ra="data:image/png;base64,",sa=document.querySelector('div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]');isOnUserProfile&&AF(document.querySelectorAll("li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl")).forEach(A=>{var e,t,n;"Birthday:"===A.children[0].innerHTML&&(e=(A=colonNumber(A.children[1].innerHTML.split(" "),2))[0],A=parseInt(A[1]??1),e=getMonthFromString(e),[t,n]=[(t=new Date).getMonth()+1,t.getDate()],A===n)&&e===t&&(sa.style.position="relative",(A=document.createElement("img")).src=ra,A.width=256,A.height=256,A.style.width="256px",A.style.height="256px",A.style.position="absolute",A.style.zIndex="999",A.style.left="100px",A.style.top="-420px",A.style.rotate="30deg",A.style.pointerEvents="none",sa.appendChild(A))})}settings.h&&isOnUserProfile&&GM_addStyle(`
        .friend>.friend-head>img { border-radius: 7px; };
    `);
