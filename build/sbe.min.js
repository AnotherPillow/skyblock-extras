...
// debugger
    ...
    `;GM_addStyle(H)}if(settings.threadTitleEnabled&&(document.title=(document.querySelector(".titleBar>h1")??document.querySelector('h1.username[itemprop="name"]'))?.textContent+" | Skyblock Forums"),settings.hideShopTab){const I=document.querySelector("ul.publicTabs"),J=I?.children[7],K=I?.children[6];J&&K?.replaceWith(J)}if(settings.strikethroughBannedUsers){const L=document.querySelectorAll(".messageUserBlock"),M=Array.from(L).filter(A=>A.querySelector('[src="styles/default/xenforo/avatars/avatar_banned_m.png"]')),N=`
        .sbe-strikethrough {
            text-decoration: line-through !important;
            text-decoration-thickness: 2px !important;
        }
    `;GM_addStyle(N),M.forEach(A=>{A.querySelector(".userText > .username")?.classList.add("sbe-strikethrough")})}if(settings.betterNewSB){const Q=`
    div.navTabs {
        background:#2b485c;
