...
if (settings.birthdayHatOnPFP) {
    ...
    const avContainer = document.querySelector(`div.profilePage[itemscope="itemscope"][itemtype="http://data-vocabulary.org/Person"] > div.mast > div.avatarScaler > span[class^="Av"]`) as HTMLElement

    if (isOnUserProfile) {
        AF(document.querySelectorAll(`li#info.profileContent > div.section > div.primaryContent > div.pairsColumns.aboutPairs > dl`))
            .forEach(_dl => {
                let dl = _dl as HTMLDListElement

                if (dl.children[0].innerHTML !== "Birthday:") return;
